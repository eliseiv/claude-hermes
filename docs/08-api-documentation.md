...
- **Требуется** (обе схемы) для пользовательских `/v1/*`: `agent` (`/v1/agent/run`, `/v1/agent/runs/{runId}/events|state|approval|stop|resume`), `chat` (`/v1/chat/run`, `/v1/chat/tool-result`), `GET /v1/tools`, `policy`, `wallet`, `subscription`, `byok`, `chats`, `profile`, `preferences`, `workspaces`, `snippets`, `attachments`, `tokens`, `notifications`. У них в Swagger UI значок замка.
...
> ⚠️ **Одноимённое поле в `GET /v1/agent/runs/{runId}/state` — другое пространство значений** ([ADR-066 §5](adr/ADR-066-agent-run-state-snapshot.md)): там `blockReason` несёт **причину паузы прогона** (`agent_runs.paused_reason`, в v1 только `credits_exhausted`) и **не валидируется** этим policy-enum'ом. Таблица выше описывает исключительно policy-причины (`ChatResponse`, `/policy/effective`, `POST /v1/agent/run`).
...
**Полная поверхность OpenAPI ([ADR-059 §7](adr/ADR-059-hermes-only-openapi-surface.md), REVERSED 2026-07-06).** В `/openapi.json` (и `/docs`, `/redoc`) документируется **весь активный API**: Hermes-ядро (`Agent`), инфраструктура (`Policy`, `Wallet`, `Tokens`, `BYOK`, `Billing (Adapty)`, `Admin`, `Health`) **и** claude-ios legacy (`Chat`, `Tools`, `Models`, `Presets`, `Preview`, `Chats`, `Workspaces`, `Profile`, `Preferences`). Причина отмены исходного «Hermes-only surface»: iOS-приложение использует НЕ только Hermes-агента, но и унаследованный claude-ios функционал (чаты, проекты/workspaces и др.), и разработчику клиента эти эндпоинты нужны видимыми в `/docs`. Механизм — снят `include_in_schema=False` с девяти legacy-роутеров (новый механизм не вводится). **Исключение — retired `/v1/auth/*`:** issuer-роутер не смонтирован (`404`, вне схемы, [ADR-044 §4a](adr/ADR-044-client-api-key-auth.md)); тег `Auth` в схеме **отсутствует**. Тег `Subscription` также отсутствует (`/v1/subscription/sync` retired, [TD-021](100-known-tech-debt.md)). Таблица ниже — все видимые теги (полный активный API минус `Auth`).

| Тег | Endpoint | Security | Описание тега (русский, кратко) |
|---|---|---|---|
| `Agent` | `POST /v1/agent/run`, `GET /v1/agent/runs/{runId}/events`, `GET /v1/agent/runs/{runId}/state`, `POST /v1/agent/runs/{runId}/approval`, `POST /v1/agent/runs/{runId}/stop`, `POST /v1/agent/runs/{runId}/resume` | `clientApiKey+userId` | Автономный ИИ-агент пользователя (Hermes). Запуск прогона, поток событий (SSE), **снапшот состояния прогона** ([ADR-066](adr/ADR-066-agent-run-state-snapshot.md)), подтверждение инструмента, остановка, **возобновление после паузы по кредитам** ([ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)). Headline-контур ([ADR-045](adr/ADR-045-hermes-as-agent-proxy.md)). |
...
| `Preview` | `GET /v1/preview/{projectId}/{token}/{path}` | none | Публичная отдача сгенерированных сайтов по подписанной ссылке (авторизация в подписи, без клиентского ключа, [ADR-010](adr/ADR-010-backend-hosted-preview.md)). |
| `Chats` | `GET/PATCH/DELETE /v1/chats[/{id}]` (+ `/{id}/steps`) | `clientApiKey+userId` | История чатов: список, переименование, закрепление, удаление, шаги. Доступ только владельца; чужой/несуществующий чат — 404. |
| `Workspaces` | `POST/GET /v1/workspaces`, `GET/PATCH/DELETE /v1/workspaces/{id}`, `POST/GET /v1/workspaces/{id}/files`, `DELETE /v1/workspaces/{id}/files/{fileId}` | `clientApiKey+userId` | Рабочие пространства (iOS «Projects»): имя, описание, инструкции и файлы-знания как контекст чатов ([ADR-036](adr/ADR-036-workspaces-implementation.md)). Доступ только владельца; чужой/несуществующий — 404. Upload файла-знания — inline base64 (`type`/`mediaType`/`filename`/`data`), ≤ 8 MB/файл; transport-лимит роута `/{id}/files` повышен до 12 MB (`WORKSPACE_REQUEST_BODY_LIMIT`, [ADR-060](adr/ADR-060-workspace-upload-transport-limit-alignment.md)) — тело > 12 MB → `413 payload_too_large`; файл > 8 MB (или число/суммарный лимит) → `413`/`422` на валидации. |
...
Каждый endpoint имеет ровно один тег из таблицы и security согласно колонке (R2). Порядок в `openapi_tags` = фактический порядок в `_OPENAPI_TAGS` (`src/app/main.py`): **`Agent`, `Chat`, `Tools`, `Models`, `Presets`, `Policy`, `Wallet`, `Tokens`, `BYOK`, `Billing (Adapty)`, `Admin`, `Preview`, `Chats`, `Workspaces`, `Profile`, `Preferences`, `Health`** ([ADR-059 §7](adr/ADR-059-hermes-only-openapi-surface.md), полная поверхность). Тег `Agent` — headline-контур, ставится перед `Chat` ([ADR-045](adr/ADR-045-hermes-as-agent-proxy.md)). `Billing (Adapty)` — в биллинг-контуре между `BYOK` и `Admin`. **Тег `Auth` в `_OPENAPI_TAGS` отсутствует** (issuer-роутер `/v1/auth/*` не смонтирован, retired, [ADR-044 §4a](adr/ADR-044-client-api-key-auth.md)). **Тег `Subscription` удалён** ([TD-021](100-known-tech-debt.md)/ревизия [ADR-029](adr/ADR-029-adapty-subscription-webhook.md)): под ним нет роутов; подписки идут через Adapty-вебхук `POST /v1/billing/adapty/webhook` (тег `Billing (Adapty)`). needs_code_sync: `_OPENAPI_TAGS` + снятие `include_in_schema=False` с девяти legacy-роутеров в `src/app/main.py`.

> Все девять ранее скрытых роутеров (`chat`, `chats`, `workspaces`, `tools`, `models`, `presets`, `profile`, `preferences`, `preview`) снова видимы в OpenAPI ([ADR-059 §7](adr/ADR-059-hermes-only-openapi-surface.md)) — регистрируются штатным `app.include_router(module.router)` без `include_in_schema=False`; их auth (`clientApiKey`+`userId`, [ADR-044](adr/ADR-044-client-api-key-auth.md)) и контракты не меняются. **`snippets`/`attachments`/`notifications` — НЕ отдельные роутеры** (в `create_app()` таких `include_router` нет): это суб-контуры/поля родителей — `attachments` подаются inline в `POST /v1/chat/run` (base64, [ADR-020](adr/ADR-020-inline-base64-attachments-mvp.md); двухшаговый `/v1/attachments` [ADR-014](adr/ADR-014-multimodal-attachments.md) не смонтирован), `snippets`/`notifications` — поля/действия в контурах `profile`/`preferences`; отдельных тегов не имеют.
...
| `GET /v1/agent/runs/{runId}/events` | описание SSE-потока (passthrough Hermes): последовательность `run.queued` → `run.running` → `message.delta` → `tool.started`/`tool.completed` → (`usage.delta`?, [ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)) → (`approval.request`?) → `run.completed{usage}` либо `run.failed` либо `run.paused` (исчерпание баланса, [ADR-064 §3](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)). Формат событий Hermes — как у Hermes ([ADR-045](adr/ADR-045-hermes-as-agent-proxy.md)); пример одного `message.delta`, одного `usage.delta{step_index,cumulative_*_tokens}`, одного `run.completed{usage:{input_tokens,output_tokens,total_tokens}}` и одного `run.paused{event:"run.paused",run_id,reason:"credits_exhausted",status:"paused",output:"<текст>",steps,billed,balance:0,usage:{cumulative_input_tokens,cumulative_output_tokens}}` (промежуточный текст — в ключе `output`, **не** `message`; ограничен `AGENT_STATE_RESULT_TEXT_MAX_CHARS` = 64 KB с сохранением начала, [ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md)). |
| `GET /v1/agent/runs/{runId}/state` | response `200` — снапшот прогона: `{runId, sessionId, status, resultText, lastTool, pendingApproval{tool,preview}, blockReason, usage{inputTokens,outputTokens}, updatedAt, continuedFrom}`. **Минимум три примера ответа:** активный прогон (`status:running`, непустой `resultText`, `pendingApproval:null`), ожидание подтверждения (`status:waiting_approval` + заполненный `pendingApproval`), пауза по кредитам (`status:paused`, `blockReason:"credits_exhausted"`). Описание роута обязано фиксировать: **read-only** (инстанс Hermes не будится, Hermes не опрашивается, кредиты не списываются), `resultText` может отставать у активного прогона без подписчика на `/events` (детект по `updatedAt`), `blockReason` здесь — **причина паузы**, а не policy-enum [ADR-004](adr/ADR-004-blocked-http-200.md). Коды: `200`, `401`, `404` (чужой/несуществующий — RBAC, 403 не бывает), `429` (rate limit). См. [modules/agent-proxy/02-api-contracts.md](modules/agent-proxy/02-api-contracts.md) ([ADR-066](adr/ADR-066-agent-run-state-snapshot.md)). |
...
| `POST /v1/agent/runs/{runId}/resume` | request (`message?`); response `202` (`status:running`, `runId`=новый, `continuedFrom`=paused runId); response `blocked` (баланс всё ещё 0/долг, HTTP 200); `409` (`run_not_resumable` — не paused/resumed \| `resume_in_progress` — конкурентный resume, повторить \| `session_expired` — hydrate 404/пусто, начать новый прогон); `404` (чужой/несуществующий, RBAC); `502` (инстанс/hydrate недоступны, CAS откачен). См. [modules/agent-proxy/02-api-contracts.md](modules/agent-proxy/02-api-contracts.md) ([ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)). |
