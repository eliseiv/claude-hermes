...
## Фикстуры внешних интеграций — только с первоисточника

**Правило (shared-блок v3).** Фикстура, описывающая форму данных **внешней** системы (событие стороннего образа, ответ third-party API, формат файла/сообщения), снимается **с первоисточника**: сырой сэмпл от самого образа/сервиса, его официальная схема или спецификация вендора. Сочинять её вместе с парсером **запрещено**, выводить из нашего кода или из `docs/` — тоже: и то и другое фиксирует наше допущение, а не контракт внешней системы, и даёт зелёный прогон при неверном коде.

**Почему правило существует (материализовавшийся дефект).** Все фикстуры `message.delta` в сюите были написаны как `{"text": …}` / `{"delta": {"text": …}}` — форма придумана вместе с парсером. Патченый образ эмитит `delta` **голой строкой**, поэтому извлекатель промахивался на **каждой** дельте, `resultText` на проде был тождественно пуст, а сюита оставалась зелёной: тесты сверяли код с его собственным допущением. Подробности — [ADR-066 §Ревизия 2026-07-29](adr/ADR-066-agent-run-state-snapshot.md), [modules/agent-proxy/05-events.md](modules/agent-proxy/05-events.md).

### Реестр капч

| Файл | Происхождение | Состав | Что НЕ покрывает |
|---|---|---|---|
| `tests/fixtures/hermes_prod_run_adr065.sse` | Капча **живого прод-прогона** `run_d931839587a64e3885b4d096cf7440d0` (devops, 2026-07-29; прогон остановлен pause-at-zero по исчерпанию баланса). Образ — [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md) | 15 × `message.delta`, 1 × `usage.delta`, 1 × `run.paused` | `run.completed` ([Q-067-10](99-open-questions.md), **денежный путь**), `tool.started`/`tool.completed`/`approval.request` ([Q-067-11](99-open-questions.md)) — при нулевом балансе и без инструментов недостижимы |
| `tests/fixtures/hermes_prod_completed_run_adr067.sse` (+ `.state.json`) | Капча прод-прогона `run_3b3b1253e0974b24b594b7452bc7095d` (devops, 2026-07-30, пополненный кошелёк, образ [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md), `gpt-5-mini`), доведённого до **штатного** `run.completed`. **6/6 блоков — побайтово, без сокращения** (весь ответ — `DONE.`). Рядом — тело `GET …/state` того же прогона | 2 × `message.delta`, 1 × `usage.delta`, 1 × `reasoning.available`, 1 × `run.completed` (вложенный `usage` = `{input_tokens:6302, output_tokens:586, total_tokens:6888}` + top-level `output`), comment-строка `: stream closed` | `approval.request` — на текущем toolset недостижим ([Q-067-11](99-open-questions.md)) |
| `tests/fixtures/hermes_prod_tool_run_adr067.sse` (2026-07-30) | Тот же образ, прогон с двумя вызовами инструментов. Сокращена удалением блоков `message.delta`: **13/27** | 2 × `tool.started` (`write_file`, `read_file`; поле `preview`), 2 × `tool.completed` (`duration`, `error`; **поля `result` НЕТ**), 3 × `usage.delta`, `reasoning.available`, `run.completed`, `: stream closed` | `approval.request` |
| `tests/fixtures/hermes_prod_no_approval_run_adr067.sse` (2026-07-30) | **Свидетельство отсутствия:** намеренная попытка спровоцировать `approval.request` (запрос деструктивного действия). Событие не возникло — агент запросил подтверждение **прозой** и завершил прогон. **10/144** | `tool.started`/`tool.completed` (`search_files`), 2 × `usage.delta`, `reasoning.available`, `run.completed`, `: stream closed` | подтверждает, что форма `approval.request` **не измерена** ([Q-067-11](99-open-questions.md)) |
| `tests/fixtures/hermes_prod_no_terminal_event_adr067.sse` и `…_2nd_sample_adr067.sse` (2026-07-30) | **Два независимых прогона** (`run_8d3b22ec…`, `run_92bf3419…`) заклинившего инстанса: поток обрывается на `usage.delta`, терминального события нет. По **4/120** блоков. Источник таймингов старта и негативный контрпример ([Q-067-6](99-open-questions.md)) | `message.delta`, `usage.delta`; **нет** `run.completed`/`run.failed`/`run.paused`, **нет** `: stream closed` | — |
| `tests/fixtures/hermes_prod_resubscribe_empty_adr067.curl.txt` (2026-07-30) | **Не SSE-капча, а транскрипт `curl`** свежей подписки после обрыва до заголовков — отсюда расширение `.txt`. Опровергает H10 ([Q-067-13](99-open-questions.md)) | `curl: (28) … 0 bytes received` за 35 с | — |

✅ **Капчи 2026-07-30 закоммичены в `tests/fixtures/`** (qa, пути в таблице выше). Верность wire-формату — удаление только целых блоков, сохранение порядка, значений, экранирования и хвостовых байт — проверяется тестом `test_adr067_fixtures_are_wire_faithful`; провенанс продублирован в докстринге `tests/unit/test_agent_sse_delta_contract_adr065.py`. Ссылки на scratchpad-имена ещё остались в [modules/agent-proxy/05-events.md](modules/agent-proxy/05-events.md) и [09-testing.md](modules/agent-proxy/09-testing.md) и в [Q-067-6/7/10/13](99-open-questions.md) — обновить на пути `tests/fixtures/…` (owner: **architect**).

**Дискриминатор штатного закрытия.** `: stream closed` — единственная comment-строка образа, одноразовая; она присутствует ровно в трёх капчах, доведённых до `run.completed`, и отсутствует в обеих капчах заклинившего инстанса ([Q-067-6](99-open-questions.md)). Признак **транспортный**: как диагностика годится, для биллинга и статуса — нет.

⚠️ **Расхождение, выявленное при коммите.** Постановка задачи описывала `q6b_1.sse` как «чистый `run.paused` + маркер `: stream closed`», а `q6b_2.sse` — как контрпример. **На диске это один и тот же случай:** два разных прогона, оба обрываются на `usage.delta`, ни в одном нет ни `run.paused`, ни маркера. Формулировки [Q-067-6](99-open-questions.md) этому НЕ противоречат (там маркер отнесён к «чисто завершившемуся потоку»), ошибка была только в постановке; тест дискриминатора построен на реальной паре «капча с `run.completed`» ↔ «капча заклинившего инстанса».

**Допустимая правка капчи — только удаление целых блоков** (здесь: часть `message.delta` из середины ответа, чтобы сократить user content). **Значения, ключи, экранирование и разделители переписывать нельзя** — иначе артефакт перестаёт быть свидетельством. Провенанс каждой капчи фиксируется здесь и в модульном `09-testing.md`, а не только в докстринге тест-модуля: доверие к фикстуре — свойство документации, а не комментария в коде.

**Следствие для приёмки.** Пока форма события не подтверждена капчей, тест, использующий её, проверяет **наше допущение**, а не контракт. Такие места обязаны быть перечислены в `unverified_external_assumptions[]` отчёта и иметь Q-номер с владельцем добычи артефакта.
