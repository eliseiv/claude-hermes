...
## Ревизия 2026-07-15 — `.156` стал ЕДИНСТВЕННЫМ deploy-target; broadnova INSTANCES-loop удалён; имя overlay-файла

**Контекст.** На момент принятия ADR-057 `.156` был описан как «**второй**» deploy-target, **дополняющий** broadnova `.154` ([ADR-017](ADR-017-shared-server-traefik-deploy.md)); §5/§7/§Совместимость ссылались на существующий broadnova `INSTANCES`-loop и co-located инстанс `claude-hermes` на `.154`. Требование изменилось: `claude-hermes` — самостоятельный сервис, и broadnova-контур из его CI удалён.

**Что изменилось (сделано devops, отражено в docs).**
1. **`.156` — ЕДИНСТВЕННЫЙ deploy-target `claude-hermes`.** Из `.github/workflows/ci.yml` + `deploy.yml` удалён deploy-job с broadnova `INSTANCES`-loop (деплоил чужие сервисы `claude-ios`/`avelyra`/`orvianix`/`veltrio` + дубль co-located `claude-hermes` на `.154`). Остался ровно один deploy-job — `deploy-avorelio` (§5). Формулировки «второй target», «дополняет ADR-017», а также §7 (совпадение имени project с co-located `.154`) и упоминания broadnova INSTANCES-loop — **исторический контекст**: co-located инстанса `claude-hermes` на `.154` из этого репозитория больше нет, «footgun совпадения имён» неактуален (единственный `claude-hermes` — на `.156`). Соответствующая ревизия [ADR-017 §Ревизия 2026-07-15](ADR-017-shared-server-traefik-deploy.md#ревизия-2026-07-15--для-claude-hermes-применяется-только-выделенный-156-adr-057).
2. **Имя overlay-файла.** Фактический overlay `.156` — **[`docker-compose.avorelio.yml`](../../docker-compose.avorelio.yml)** (не `docker-compose.traefik.yml`, как в §3/§Последствия immutable-тела). Механизм тот же: явный `networks.web.external: false` (+ `name: web`) переопределяет унаследованный `external: true` базового файла; пустой `web: {}` недостаточен. Деплой — `-f docker-compose.prod.yml -f docker-compose.avorelio.yml`.

Инварианты §1–§4 (self-hosted Traefik, ACME `le`/HTTP-01, `docker.sock :ro` в traefik+api, preview pass-through, `TRUSTED_PROXY_IPS` = подсеть нашей `web`) и §6 (prod-checklist `.156`) **без изменений**. Тело ADR-057 не переписано (immutability). Playbook — [07-deployment.md §Топология](../07-deployment.md#топология--выделенный-сервер-156-avorelio-self-hosted-traefik-adr-057).

## Ревизия 2026-07-15 (2) — split-up: `--force-recreate` ТОЛЬКО для `api` (фикс stale-code на зелёном деплое)

**Контекст (прод-инцидент).** §5 и §Rollback описывали шаг старта как единый `docker compose up -d --no-build`. Обнаружен тихий дефект деплоя: `docker compose up -d` решает пересоздавать ли контейнер по **CONFIG-hash сервиса**, который ссылается на образ по **стабильному тегу** `${COMPOSE_PROJECT_NAME}-backend:prod` (= `claude-hermes-backend:prod`), а **не** по resolved image id. Пересборка образа под тем же тегом (шаг `build`) **не меняет** config-hash → простой `up -d --no-build` без явного сервиса видит «no change» и **оставляет старый контейнер `api`** (старый код), несмотря на новый образ и применённую миграцию. Readiness-gate по `GET /ready` (db+redis) оставался зелёным — он **не отражает версию кода** — поэтому деплой «зелёный», а на проде исполнялся старый код (новые API-поля возвращали `null`).

**Что изменилось (сделано devops в `ci.yml` `deploy-avorelio` + `deploy.yml`, отражено в [07-deployment.md §Процедура деплоя](../07-deployment.md#процедура-деплоя-github-actions--ssh-156) и §Откат).** Единый `up -d --no-build` шага старта разделён на **две** команды:
1. **(3a) обычный up stateful/edge:** `up -d --no-build --no-deps postgres redis traefik` — БЕЗ `--force-recreate` (стартует, только если не запущены; уже healthy — не трогает). `--no-deps`, чтобы `traefik` (`depends_on: api`) не втянул `api`.
2. **(3b) force-recreate ТОЛЬКО api:** `up -d --no-build --no-deps --force-recreate api` — безусловное пересоздание единственного stateless-сервиса `api`, чтобы принять свежесобранный образ независимо от неизменного config-hash. `--no-deps` гарантирует, что `--force-recreate` не заденет stateful/edge.

**Инвариант деплоя (новый, обязательный):** `postgres`/`redis`/`traefik` **НИКОГДА** не под `--force-recreate` — их безусловное пересоздание грозит потерей данных / downtime (`postgres`), сбросом кэша (`redis`) и переоформлением/потерей Let's Encrypt/ACME state `acme.json` + переинициализацией владельца `:443` (`traefik`). Пересоздаётся только `api`. Тот же split применяется к **rollback** (§Rollback: `up -d --no-build --no-deps --force-recreate api` вместо простого `up`) — тот же config-hash footgun для откаченного образа.
