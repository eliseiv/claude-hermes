...
- ⚠️ **Ревизия 2026-07-29 → [ADR-067](ADR-067-agent-run-background-consumer.md):** §3 читается как «клиентский `/events` открывает SSE к инстансу Hermes и ретранслирует» — **устарело при `AGENT_RUN_CONSUMER_ENABLED=true`**. К Hermes подключается **только фоновый consumer**; клиент читает **downstream** из Redis-ring + pub/sub. Причина — измеренный факт: прогон без подписчика не тарифицировался и вечно висел `running` (3 прод-прогона → 0 списаний, [TD-037](../100-known-tech-debt.md)), а биллинг не может зависеть от присутствия клиента. **Форма событий не меняется** (те же типы, тот же wire-формат `data: {json}` с полем `event`), но **семантика подключения меняется и требует согласования с iOS:** каждое соединение `/events` теперь **самодостаточно и начинается с реплея с начала прогона**, поэтому клиент обязан сбрасывать накопленный текст при новом подключении ([ADR-067 §3.4](ADR-067-agent-run-background-consumer.md)). Тело ADR-045 не переписано.
...
- Биллинг происходит **после** генерации (на `run.completed`); при разрыве SSL до `run.completed` списание не произойдёт на этом соединении — ~~митигация: idempotency по `runId` + повторная подписка на events / реконсиляция~~ ([Q-047-2](../99-open-questions.md)). ⚠️ **Ревизия 2026-07-29:** «повторная подписка на events» митигацией **не является** — она предполагает, что клиент переподключится, а измерение показало, что он этого не делает (3 прод-прогона → 0 списаний). Idempotency по `runId` защищает от **двойного** списания, но не восстанавливает **несделанное**. Дополнительно существует **неподтверждённая** гипотеза, что переподписка получает 0 байт ([Q-066-1](../99-open-questions.md), H1) — на негодность митигации она не влияет ни при каком исходе. Измеренная цена: 3 прод-прогона → 0 списаний ([TD-037](../100-known-tech-debt.md)). Реальная митигация — фоновый consumer ([ADR-067](ADR-067-agent-run-background-consumer.md)), тарифицирующий прогон независимо от клиента.
