...
- **Ревизия 2026-07-15 → [ADR-065](ADR-065-patched-hermes-image-ghcr.md) (источник `HERMES_IMAGE`):** §Context «Docker-образ Hermes» и §1 `provision` (`docker run` образа) читаются как **upstream публичный `nousresearch/hermes-agent`** — это **устарело**. Для incremental-биллинга ([ADR-064 §7](ADR-064-incremental-agent-run-billing-and-pause-resume.md)) нужен per-step `usage.delta` + hydrate-эндпоинт, которых в upstream **нет**. Актуальный источник — **наш патченый образ `ghcr.io/eliseiv/hermes-agent@sha256:<digest>`** (digest-pin, НЕ `:latest`), собираемый **off-server** и pull'имый на `.156` ([ADR-065](ADR-065-patched-hermes-image-ghcr.md)). Инварианты жизненного цикла/env/`config.yaml`/toolset/владения томом (§1–§6 тела) — **без изменений** (патч аддитивен: SSE-событие + read-эндпоинт). Тело ниже не переписано (immutability); ссылки на `nousresearch/hermes-agent` как источник — см. ADR-065.
