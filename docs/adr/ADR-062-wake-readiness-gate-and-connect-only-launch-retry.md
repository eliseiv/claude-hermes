...
**Это не теоретическая оценка — ровно она измерена на проде:** `POST /v1/agent/run` висел **≥90 с без ответа** (`HTTP=000`) — [TD-040](../100-known-tech-debt.md). ⚠️ **Атрибуция «на заклиненном инстансе» снята:** причиной оказался **наш собственный row-lock**, удерживавшийся через сетевой вызов, а не немота инстанса ([TD-039](../100-known-tech-debt.md) переатрибутирован, [ADR-067 §Context](ADR-067-agent-run-background-consumer.md) — ~~Факт 3~~ отозван). На вывод настоящего ADR это не влияет: умножение времени отказа втрое даёт **любая** неограниченная попытка connect, независимо от того, что её задерживает. Механизм, введённый этим ADR как защита от транзиентных сбоев, при недешёвом connect **умножал** время отказа втрое.
