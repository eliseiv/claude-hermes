...
> ⚠️ **Ревизия 2026-07-29 ([ADR-067 §1](ADR-067-agent-run-background-consumer.md)).** Рецепт «переподключиться к `/events` и догнать» негоден как гарантия: он требует действия клиента, а измеренный факт (3 прогона → 0 списаний, прогон висит `running`) показывает, что клиент переподключается не всегда. Есть также **неподтверждённая** гипотеза H1, что переподписка получает `0` байт ([Q-066-1](../99-open-questions.md)) — она усилила бы вывод, но он от неё не зависит. Отложенный здесь фоновый consumer принят [ADR-067](ADR-067-agent-run-background-consumer.md) в broker-форме.
...
2. **Replay-guard — per-column, НЕ row-level.** Hermes при переподключении к `/events` реплеит буфер **с начала**, поэтому второй потребитель может начать с короткого префикса и затереть более полный текст. Защита — монотонность **отдельных колонок**:
   ...
   > ⚠️ **Ревизия 2026-07-29 ([Q-066-1](../99-open-questions.md)).** Посылка «Hermes реплеит буфер с начала» — **непроверенное допущение** (H4 в [ADR-067](ADR-067-agent-run-background-consumer.md)): прод-процедура её не упражняла (подписка открывалась сразу после старта прогона, реплеить было нечего). Наблюдение прода — иное: вторая **последовательная** подписка получила `0` байт, но живость прогона в тот момент не зафиксирована, поэтому дренаж (H1) **не доказан**. **Практический вывод не меняется:** префиксный guard корректен при любом исходе — при реплее-с-начала он работает как проверка длины, при отсутствии реплея второй потребитель просто ничего не пишет. SQL ниже остаётся в силе как есть и после [ADR-067](ADR-067-agent-run-background-consumer.md), где writer'ом становится consumer; меняется только статус обоснования — с «защиты от известной семантики» на defense-in-depth под неизвестную.
...
> 1. **«idempotency по `runId` довыполнит debit при повторной подписке» — не гарантия, а надежда на действие клиента.** Измерено: 3 прогона → 0 списаний, `trialRemaining`=1, прогон висит `running` >15 мин — клиент не переподключился ни разу. Idempotency защищает от **двойного** списания, но не восстанавливает **несделанное**. (Есть также **неподтверждённая** гипотеза H1, что переподписка вообще получает `0` байт — [Q-066-1](../99-open-questions.md); вывод от неё не зависит.)
> 2. **«отложен» — более не отложен.** [ADR-067](ADR-067-agent-run-background-consumer.md) принимает consumer в **broker-форме**. ⚠️ Формулировка «наивный параллельный consumer физически невозможен», записанная в первой редакции ADR-067, **снята как недоказанная** — она требует гипотезы H2 (одновременные подписчики), которая **не измерялась** ([Q-067-5](../99-open-questions.md)). Broker выбран по основаниям, не зависящим от поведения образа ([ADR-067 §0](ADR-067-agent-run-background-consumer.md)).
>
...
- **`resultText` активного прогона отстаёт**, пока никто не подписан на `/events` (§1). Митигация — `updatedAt` в ответе + рецепт клиента: `GET /state` → если `running`/`waiting_approval` → переподключиться к `/events` и догнать. ⚠️ **Ревизия 2026-07-29:** рецепт негоден как гарантия — он требует действия клиента, которого измеренно не происходит (а по неподтверждённой гипотезе H1 переподписка ещё и получает `0` байт, [Q-066-1](../99-open-questions.md)). После [ADR-067](ADR-067-agent-run-background-consumer.md) отставание исчезает по существу: снапшот двигает consumer, а не клиент, и переподключение к `/events` даёт реплей из ring.
...
| §6: «Hermes при переподключении реплеит буфер **с начала**» | Прод-процедурой **не упражнялось** (подписка открывалась сразу после старта — реплеить нечего) | **Непроверенное допущение (H4)** | §6 (маркер), [Q-066-1](../99-open-questions.md) |
| §6: семантика replay — «с начала» либо «только новые» | Наблюдение: вторая **последовательная** подписка получила **0 байт**. Интерпретация «дренаж» **не доказана** — живость прогона в тот момент не зафиксирована, сырых артефактов нет | **Наблюдение + гипотеза (H1)** | [Q-066-1](../99-open-questions.md) (Partially Closed), [ADR-067 §Непроверенные внешние допущения](ADR-067-agent-run-background-consumer.md) |
| §Alternatives 1: фоновый consumer «независимый от клиента» | Заменён на broker. ⚠️ Обоснование «параллельный consumer физически невозможен» **снято**: требует гипотезы H2 (одновременные подписчики), которая **не измерялась** | **Решение принято по другим основаниям** ([ADR-067 §0](ADR-067-agent-run-background-consumer.md)) | §Alternatives 1 (маркер), [Q-067-5](../99-open-questions.md) |
