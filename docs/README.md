...
- **Мульти-инстанс / клонирование (2026-06-10, [ADR-017 §Мульти-инстанс](adr/ADR-017-shared-server-traefik-deploy.md) расширение):** несколько изолированных инстансов одного кода claude-ios за общим Traefik под разными доменами (1-й — `broadnova.shop`, 2-й — `avelyraweb.shop`). Параметризация `docker-compose.prod.yml` через `${COMPOSE_PROJECT_NAME:-claude-ios}` (image + Traefik router/service-имена) с **инвариантом обратной совместимости** (дефолт=`claude-ios`, `compose config` идентичен для существующего `.env` — живой broadnova не затронут); per-instance JWT keypair в `/opt/<inst>/.secrets/`, свежие секреты, INSTANCES-loop в CI. Playbook провижининга клона — [07-deployment.md §Мульти-инстанс](07-deployment.md#мульти-инстанс--клонирование-сервиса); статус выката 2-го инстанса — [Q-017-3](99-open-questions.md). Внесение compose/CI-параметризации — next-step devops.
