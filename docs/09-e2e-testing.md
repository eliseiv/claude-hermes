...
- `docker pull ghcr.io/eliseiv/hermes-agent@sha256:<digest>` (наш патченый образ, [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md); при приватном ghcr-пакете — после `docker login ghcr.io`) + задать этот digest-ref в `HERMES_IMAGE` (пусто → `provision` fail-fast; **digest-pin, не `:latest`**, сборки из исходников на сервере нет). Incremental-биллинг ([ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)) требует именно патченого образа (`usage.delta` в upstream отсутствует);
