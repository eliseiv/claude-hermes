...
- **`provision`** — сгенерировать `API_SERVER_KEY` (CSPRNG ≥16 симв.), записать `hermes_instances` `status=provisioning` (commit — арбитр гонки), `RuntimeBackend.provision(...)` (см. ниже, env включает `HERMES_UID`/`HERMES_GID`), **readiness-poll `health(endpoint, api_key)` до `200`** (интервал `HERMES_PROVISION_READY_INTERVAL_SECONDS`, бюджет `HERMES_PROVISION_READY_TIMEOUT_SECONDS`), **затем** `mark_running` (`provisioning→running`), зашифровать ключ (`byok.kms`). **Таймаут readiness → cleanup** (`RuntimeBackend.remove` + подчистка строки, без неконсистентной `running`/залипшей `provisioning`) → `UpstreamError`. Том сохраняется (удаляется только контейнер). Контракт readiness/ownership — [ADR-056](../../adr/ADR-056-provision-readiness-gate-and-volume-ownership.md).
  ...
  - **Readiness-gate (wake, [ADR-062](../../adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md)):** тот же poll применяется в ветке `status=stopped` — после `RuntimeBackend.start` (`docker start`) и **до** `mark_running` (ранее его НЕ было → `502` на неготовый `api_server`). Коротко-транзакционно: `T=now(UTC)` → под row-lock `start` → `mark_provisioning(user_id, provisioning_started_at=T)` (`stopped→provisioning`, сохраняя `container_id`/`endpoint`/`port`; выставляет stale-якорь `provisioning_started_at=T`) → **commit** (арбитр гонки, освобождение lock) → `_wait_for_ready` без DB → `mark_running`. **Таймаут wake → УСЛОВНЫЙ re-hibernate** `mark_stopped_if_provisioning(user_id, provisioning_started_at=T)`: rowcount=1 → `RuntimeBackend.stop` best-effort + `502`; rowcount=0 (конкурентный replay/provision завладел строкой) → не трогаем → `502` (анти-clobber чужого `running`, [ADR-062 §1b](../../adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md)). Контейнер НЕ удаляется — валиден; том цел. Reuse `HERMES_PROVISION_READY_*` (без новых знобов; wait ≤ ready < stale — инвариант истинен благодаря якорю `provisioning_started_at`, НЕ `created_at`, [ADR-062 §1a](../../adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md)). Сигнал `/health` достаточен ([ADR-062](../../adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md): маршруты образа на одном listener до `TCPSite.start()`).
