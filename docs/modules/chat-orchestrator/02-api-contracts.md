...
### Auth
- **Клиентский контур** (как все `/v1/*`, кроме `/v1/preview/*`): активно — `X-API-Key` + `X-User-Id` ([ADR-044](../../adr/ADR-044-client-api-key-auth.md)/[ADR-058](../../adr/ADR-058-x-user-id-string-identity.md)); прежде — `Authorization: Bearer <JWT>` (спящий контур). Каталог не секретен, но единообразие gateway-auth и снижение анонимного API-surface — обоснование в [ADR-019](../../adr/ADR-019-tools-catalog-endpoint.md). Токен получать не нужно: issuer `/v1/auth/*` retired (не смонтирован → `404`, [ADR-044 §4a](../../adr/ADR-044-client-api-key-auth.md)).
