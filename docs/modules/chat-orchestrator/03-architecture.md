...
2. Вызвать **Policy Engine** `evaluate(state, mode)`.
   ...
   - **Trial-claim до генерации ([ADR-054](../../adr/ADR-054-trial-claim-reconcile.md), закрывает [TD-006](../../100-known-tech-debt.md)):** если policy-allow по trial-ветке (`mode=credits`, `subscription=none`, `trial_used=false`) — **до** генерации в **отдельной короткой транзакции** выполнить атомарный условный claim: `UPDATE users SET trial_used=TRUE WHERE id=:uid AND trial_used=FALSE RETURNING id; COMMIT`. `affected=1` → этот запрос выиграл trial, выставить флаг `claimed_trial=true`, продолжить. `affected=0` → trial уже занят (гонка/прошлый ход) → вернуть `200 {status:blocked, blockReason=trial_used}` (повторная policy-оценка для актуального состояния), **без** генерации. Это **сериализует** параллельные первые запросы (атомарный UPDATE — единственный арбитр гонки; **НЕ** advisory lock и **НЕ** одна транзакция с generate-loop — совместимо с commit-до-LLM, MAJOR-4). Прежний flip `mark_trial_used` после успешной генерации (§8) для trial-ветки **снимается** — trial теперь флипается здесь.
...
8. **Списание кредитов** (`mode=credits`, [ADR-006](../../adr/ADR-006-credit-billing-and-subscription-grant.md)):
   ...
   - При `status=blocked` с `blockReason=max_tokens` (обрезка, [ADR-025](../../adr/ADR-025-parallel-tool-calls-and-max-tokens-truncation.md)) списание **НЕ выполняется** (обрыв — не успешный финальный assistant_message). Пользователь не платит за оборванную генерацию.
...
8a. **Trial-reconcile при неуспехе ([ADR-054](../../adr/ADR-054-trial-claim-reconcile.md)).** Если на шаге 2 был выполнен trial-claim (`claimed_trial=true`), а ход **НЕ** завершился успешным `status=assistant_message` (т.е. `status=blocked/max_tokens`, `UpstreamError`/`502`, исключение, отмена) — **откатить claim** в отдельной короткой транзакции: `UPDATE users SET trial_used=FALSE WHERE id=:uid AND trial_used=TRUE`. Сохраняет семантику «trial сгорает только при УСПЕШНОЙ генерации» (симметрия с max_tokens-неначислением). На успехе (`assistant_message`) reconcile **не** выполняется — trial остаётся `TRUE`. Промежуточный `status=tool_call` — НЕ неуспех, claim остаётся до финализации. Реализация — `try/except/finally` вокруг generate-loop с флагом `claimed_trial`. Остаточный риск «потерянного trial» при краше в окне claim↔reconcile — [ADR-054 §4](../../adr/ADR-054-trial-claim-reconcile.md)/[Q-054-1](../../99-open-questions.md).
9. Audit шага.
...
6. Re-evaluate Policy (доступ мог измениться); продолжить с шага 3 потока run (повторный вызов Anthropic). При сборке messages **все** `tool_result`-блоки хода (client-side из этого/прошлых батчей + server-side, исполненные на бэке) формируются с `tool_use_id = tool_calls.provider_tool_use_id` (НЕ доменный UUID, НЕ свежий uuid4) — см. [§ Согласованность tool_use.id](#согласованность-tool_useid-в-истории-anthropic-bug-4). Continuation-виток выполняется **один раз** на закрытие барьера.
   - **Trial-continuation под claimed trial ([ADR-054 §2a](../../adr/ADR-054-trial-claim-reconcile.md), закрывает кейс [TD-006](../../100-known-tech-debt.md) на пути `/chat/tool-result`):** trial-ход мог вернуться `status=tool_call` с **client-side** инструментами на `/chat/run` (claim §2 уже выполнен, `trial_used=TRUE`, финального `assistant_message` ещё нет). Тогда re-evaluate этого шага даёт `blocked(trial_used)` (policy — чистая функция) и continuation **ошибочно** заблокировался бы **своим же** claim'ом (регрессия claim-before: до ADR-054 flip был после хода). **Признак in-progress trial-хода** (решается **в orchestrator**, НЕ в engine — policy не меняется): `mode=credits AND subscription=none AND trial_used=TRUE AND assistant_tool_step_id(session_id, message_step_id) is not None`. Детерминизм опирается на **инвариант пути**: re-evaluate в `tool_result` достигается только после хода, вернувшего `tool_call`, и только при закрытии барьера — поэтому любой assistant-шаг этого `message_step_id` здесь **по построению** является tool_use-ходом, и его существование означает, что именно **этот** ход застолбил свой trial (≠ новый `/chat/run`, заблокированный прошлым trial — другой кодовый путь). `assistant_tool_step_id` (`src/app/chat/repository.py`) фильтрует только `role='assistant'` (без отдельной проверки `tool_use`-блоков), что на этом пути эквивалентно «tool_use-ход существует». **Правило:** если `block_reason == trial_used` И признак истинен — **НЕ** блокировать, продолжить generate-loop. Любой иной `block_reason` (`credits_empty`/`subscription_*`/`byok_*`/`rate_limited`) — блок как прежде. Признак заменяет request-scoped флаг `claimed_trial` (он не переносится между запросами).
   - **Trial-reconcile на continuation ([ADR-054 §2a](../../adr/ADR-054-trial-claim-reconcile.md)).** Тем же признаком `is_inflight_trial_turn` continuation-ход распознаётся как trial-turn. Семантика — идентична run §8a: если continuation **НЕ** завершился `assistant_message` (`status=blocked`/`max_tokens`, `UpstreamError`/`502`, исключение, отмена) И ход был trial-turn → **откатить claim** в отдельной короткой транзакции `UPDATE users SET trial_used=FALSE WHERE id=:uid AND trial_used=TRUE` (идемпотентно, «не трогать чужой trial»). `assistant_message` → reconcile не выполняется (claim остаётся). Промежуточный `status=tool_call` (следующий round того же хода) — НЕ неуспех, claim остаётся до финализации. Биллинг неизменен: на continuation `trial_used` уже `TRUE` → `_billing_plan` даёт `mark_trial=FALSE` (повторного flip нет — claim уже застолбил trial на `/chat/run`). Признак — из существующих данных (`users.trial_used` + assistant-tool_use-шаг хода), **без миграции**; публичный контракт `/chat/tool-result` не меняется. Остаточный риск «потерянного trial» — [ADR-054 §4](../../adr/ADR-054-trial-claim-reconcile.md)/[Q-054-1](../../99-open-questions.md).
...
4. **Биллинг:** кредит **не** списывается (обрыв — не успешный финальный `assistant_message`). **Trial ([ADR-054](../../adr/ADR-054-trial-claim-reconcile.md)):** если был trial-claim до генерации (§2), он **откатывается** reconcile-шагом (§8a) — `max_tokens`-обрыв возвращает trial пользователю (trial сгорает только при успехе). См. [§Биллинг кредитов](#биллинг-кредитов-правило-списания) / поток run §8a.
