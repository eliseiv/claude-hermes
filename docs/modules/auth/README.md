...
> **⚠️ HTTP-поверхность `/v1/auth/*` — RETIRED (не смонтирована), КОД — спящий ([ADR-044 §4a](../../adr/ADR-044-client-api-key-auth.md), ревизия 2026-07-06).** Issuer-роутер `/v1/auth/*` НЕ регистрируется в `create_app()` → `register`/`token`/`refresh`/`apple`/`jwks` дают `404` (не `200`/`503`). Клиентская идентичность `/v1/*` — только `X-API-Key` + `X-User-Id` ([ADR-044](../../adr/ADR-044-client-api-key-auth.md)/[ADR-058](../../adr/ADR-058-x-user-id-string-identity.md)). **Сохраняются (живой код):** issuer/Apple-верификатор (`src/app/auth/*`), таблицы `auth_devices`/`auth_refresh_tokens`/`auth_identities` (миграции `0005`/`0012`), cleanup-reaper ([TD-013](../../100-known-tech-debt.md), в `lifespan`) — их **unit/миграционные/reaper-тесты зелёные**. **Ретайрены:** интеграционные наборы, бившие по live-эндпоинтам issuer'а (`test_auth_api.py`/`test_auth_apple_api.py`/`test_auth_unconfigured.py`). Разделы ниже описывают issuer как **исходно спроектированный** (путь будущего апгрейда идентичности) — статусы `503`/`200` относятся к спящему коду, а не к текущей HTTP-поверхности.

- Статус: **Код реализован, но HTTP-поверхность спящая** (MVP-расширение, 2026-06-02; **Sign in with Apple реализован 2026-06-19, [ADR-043](../../adr/ADR-043-sign-in-with-apple.md)**). Эндпоинты register/token/refresh/**apple**/jwks, device-based identity, refresh-token rotation, кросс-девайс через `auth_identities`; миграции `0005` (auth) + `0012_auth_identities` (single head). Unit/миграционные тесты зелёные. **Текущее HTTP-поведение: `/v1/auth/*` retired — роутер не смонтирован → `404`** (не `503`; [ADR-044 §4a](../../adr/ADR-044-client-api-key-auth.md)). RSA-пара подписи и `APPLE_AUDIENCE` — конфиг спящего issuer'а, для публичного запуска **НЕ требуются** (нужны только при аддитивной реактивации); статусы `503`/`200` ниже относятся к спящему коду, а не к текущей поверхности.
...
- Верификация выпущенных токенов — существующим `JwtVerifier` (`src/app/api_gateway/auth.py`), **без изменения** его логики (тот же RS256, `iss`/`aud`/`exp`/`sub`). Issuer/audience — собственные (`https://avorelio.shop` / `claude-hermes`).
