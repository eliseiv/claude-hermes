...
> |---|---|---|---|
> | Заголовок `Last-Event-ID` | `<epoch>-<seq>`, как отдан в `id:` | **выше** | трактуется как **пустой курсор** (заголовок ставит SSE-библиотека — ронять реконнект из-за него нельзя) |
> | Query `?afterSeq=` | целое **≥ 0** | ниже; **игнорируется**, если пришёл `Last-Event-ID` | **`400`** (не целое / отрицательное) |
>
...
> | Клиент | Поведение |
> |---|---|
> | Шлёт `Last-Event-ID` (стандартная SSE-библиотека) или `?afterSeq=N` | получает **только новые** события — дублей нет, накопленный текст сбрасывать **не нужно** |
...
1. ~~**`resultText` активного прогона отстаёт**, пока к `/events` никто не подключён. Рецепт клиента: `GET /state` → если `running`/`waiting_approval` → **переподключиться к `/events`** и догнать поток; свежесть снапшота видна по `updatedAt`.~~ ⚠️ **Ревизия 2026-07-29.** До [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md) ограничение было **строже**, чем описано, а рецепт — ненадёжен: он требует, чтобы клиент переподключился, а измерено, что этого не происходит (3 прогона → 0 списаний, прогон висел `running` >15 мин, [TD-037](../../100-known-tech-debt.md)). Наблюдение того же замера: вторая последовательная подписка на `/events` получила **0 байт** — интерпретация этого наблюдения (дренаж буфера) **остаётся непроверенной гипотезой** ([Q-066-1](../../99-open-questions.md), перемер назначен), поэтому строить на ней клиентские рецепты нельзя. **После [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md):** снапшот двигает фоновый consumer, поэтому `resultText` растёт **и без подписчика** — ограничение снимается; остаточное отставание — не более `AGENT_STATE_FLUSH_INTERVAL_SECONDS` (3 с).
