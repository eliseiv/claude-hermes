...
### Поведение
- **404** — прогон не существует **либо принадлежит другому субъекту** (неотличимы; `403` не возвращается никогда). ⛔ **Проверка выполняется в ОБРАБОТЧИКЕ маршрута, до создания `StreamingResponse`.** Причина — порядок ASGI: Starlette отправляет `http.response.start` **до** первого `__anext__` генератора, поэтому проверка внутри генератора не может дать статус вовсе (`404` физически невыразим, контракт статуса ломается; данные при этом не утекают). ⚠️ **По тому же основанию в обработчике живёт и валидация `?afterSeq=` (`400`)** — обе проверки этого маршрута стоят там не по разным причинам, а по одной. В генераторе проверка владения **сохранена** как защита в глубину для прямых вызывающих (тесты, будущие не-HTTP потребители): авторитетный `404` даёт обработчик, генератор страхует ([06-rbac.md](06-rbac.md)). ⚠️ **Под broker-моделью это ЯВНАЯ проверка по `agent_runs`, а не следствие адресации инстанса** — broker читает Redis по `run_id` из пути, и прежняя гарантия «чужой `runId` недостижим по построению» здесь **не действует** ([06-rbac.md](06-rbac.md)).

> **Источник событий — broker, не Hermes ([ADR-067](../../adr/ADR-067-agent-run-background-consumer.md), при `AGENT_RUN_CONSUMER_ENABLED=true`).** Эндпоинт **не подключается к инстансу Hermes**: он читает Redis-ring прогона (реплей с начала) и канал pub/sub (live), которые наполняет фоновый consumer — единственный upstream-подписчик. Серверные отличия: (1) эндпоинт **ничего не пишет** — ни списаний, ни снапшота, ни статуса; (2) прогон тарифицируется и получает терминальный статус **даже если клиент не подписался вовсе** ([TD-037](../../100-known-tech-debt.md)); (3) на очень длинном прогоне (> `AGENT_RUN_EVENT_BUFFER_MAX` событий или > `AGENT_RUN_EVENT_BUFFER_MAX_BYTES`) реплей может начаться **не с начала** — голова добирается через `GET …/state.resultText` ([Q-067-3](../../99-open-questions.md)).
>
> **Реконнект по курсору ([ADR-067 §3.4](../../adr/ADR-067-agent-run-background-consumer.md)).** Каждое событие сопровождается SSE-полем **`id: <epoch>-<seq>`** — непрозрачным курсором (разбирать его клиенту не нужно).
>
> **Приём курсора (точный контракт):**
>
> | Источник | Формат | Приоритет | Невалидное значение |
> |---|---|---|---|
> | Заголовок `Last-Event-ID` | `<epoch>-<seq>`, как отдан в `id:` | **выше** | трактуется как **пустой курсор**, **НЕ ошибка** (заголовок ставит SSE-библиотека, а не автор кода — ронять реконнект из-за поведения его рантайма нельзя) |
> | Query `?afterSeq=` | целое **≥ 0** | ниже; **игнорируется**, если пришёл `Last-Event-ID`. ⚠️ **Это ФИЛЬТР реплея, а не курсор дедупа** ([ADR-067 §3.2.1](../../adr/ADR-067-agent-run-background-consumer.md)): он не несёт `epoch`, поэтому по поколению неверифицируем, **всегда** сопровождается `run.truncated` и **никогда** не подавляет live-события. Значение выше фактического счётчика ⇒ курсор считается **пустым** (полный реплей + маркер), а не «всё уже отдано». Клиентам рекомендуется `Last-Event-ID` — его ставит SSE-библиотека и он самоидентифицирующий; целочисленная форма остаётся для отладки | **`400`** `bad_request` (не целое / отрицательное) — ⚠️ **не `422`**: это не «тело не прошло схему», а синтаксически негодный параметр запроса |
>
> **Пустой курсор ≡ `0`** — то же, что его отсутствие: отдаётся полный реплей ring'а.
>
> **Курсор считается пустым также, если:** `epoch` не совпал с текущим поколением ring'а (перезапуск Redis, `FLUSHDB`, истечение TTL) либо `seq` больше текущего максимума. В этих случаях клиент получает полный реплей того, что есть, **предварённый `run.truncated`** — вместо молчащего стрима.
>
> **Смена поколения под уже открытым соединением** обрабатывается так же ([ADR-067 §3.3.1](../../adr/ADR-067-agent-run-background-consumer.md)): сервер сверяет поколение не только при открытии, но и на каждом событии, поэтому клиент, подключённый в момент перезапуска Redis, получает `run.truncated` и продолжение потока, а не тишину. Со стороны клиента специальной обработки не требуется — правило то же: увидел `run.truncated` ⇒ поток не полон, накопленный текст брать в `GET …/state`.
>
> | Клиент | Поведение |
> |---|---|
> | Шлёт `Last-Event-ID` (стандартная SSE-библиотека) | получает **только новые** события — дублей нет, маркера нет, накопленный текст сбрасывать **не нужно** |
> | Шлёт **`?afterSeq=N`, `N > 0`** | ⚠️ **не эквивалент заголовка** ([ADR-067 §3.2.1](../../adr/ADR-067-agent-run-background-consumer.md)): параметр не несёт поколения, поэтому он — **фильтр реплея**, а не курсор дедупа. Live-события им **не подавляются**, отдача **всегда** предваряется `run.truncated` ⇒ клиент обязан взять накопленный текст из `GET /v1/agent/runs/{runId}/state`. Для инкрементального реконнекта **без** маркера используйте `Last-Event-ID` |
> | Шлёт `?afterSeq=0` | тождественно отсутствию курсора: полный реплей, маркер **только** если ring уже обрезан |
> | Курсор не поддерживает | получает **полный реплей** → обязан **сбросить накопленный текст** при каждом новом соединении, иначе удвоит его |
>
> **`run.truncated` — синтетическое событие, обязательное в v1.** Отдаётся **перед** реплеем, если первый отданный `seq` больше `курсор + 1` (при пустом курсоре — если первый `seq` > 1): `data: {"event":"run.truncated","run_id":"…","from_seq":N,"reason":"buffer_trimmed"}`. ⚠️ **Его `id:` несёт `seq` КУРСОРА (`0` при пустом), а не первого события** — маркер не занимает номер в последовательности, иначе клиент запомнил бы его как курсор и при следующем реконнекте **пропустил первое реальное событие**. Причины разрыва — обрезка ring'а (`AGENT_RUN_EVENT_BUFFER_MAX` / `..._MAX_BYTES`) либо смена поколения. Семантика: **начало потока недоступно, полный накопленный текст брать в `GET …/state.resultText`**. ⚠️ **Полноту текста длинного прогона не гарантирует ни один источник:** `/state.resultText` — 64 KB head-preserving, `/events` — содержимое ring'а; неполнота при этом наблюдаема (маркер), а не молчалива.
>
> ⚠️ **Что было записано ранее и снято:** «переподписаться на `/events`, чтобы довыполнить debit / догнать текст». Гарантию биллинга нельзя строить на действии клиента — измерено: 3 прогона → 0 списаний, клиент не переподключился ни разу ([Q-047-2](../../99-open-questions.md)).

### Правила закрытия стрима ([ADR-067 §3.3](../../adr/ADR-067-agent-run-background-consumer.md))
Терминальное событие может не появиться никогда (consumer не встал/умер, TTL ring истёк, Redis перезапущен, прогон терминален давно). Раньше стрим закрывал сам Hermes; теперь закрываем мы — при **любом** из условий: получено терминальное событие; `agent_runs.status` уже терминален при открытии; нет живого lease **и** ring пуст; статус стал терминальным во время стрима (периодическая сверка); `AGENT_RUN_DOWNSTREAM_IDLE_TIMEOUT_SECONDS` без событий и без lease. **Клиент не должен висеть бесконечно ни в одном из этих случаев.**

- Отдаёт клиенту события прогона: `run.queued|run.running|message.delta|tool.started|tool.completed|approval.request|usage.delta|run.completed|run.failed|run.paused`. **Легаси-путь** (`AGENT_RUN_CONSUMER_ENABLED=false`, [TD-038](../../100-known-tech-debt.md)): открывает SSE напрямую к `GET {base}/v1/runs/{runId}/events` и несёт биллинг/снапшот сам — поведение, описанное ниже.
...
- Идемпотентность по `runId` ([ADR-005](../../adr/ADR-005-idempotency-ledger.md)): повторный вызов/ретрай → один debit. ⚠️ **Не читать это как «повторная подписка довыполнит пропущенный debit»** — идемпотентность защищает от **двойного** списания, а не восстанавливает **несделанное**, и гарантия, требующая переподключения клиента, негодна: измерено 3 прогона → 0 списаний, клиент не переподключился ни разу ([Q-047-2](../../99-open-questions.md)). Гарантию «списание произойдёт» даёт consumer ([ADR-067](../../adr/ADR-067-agent-run-background-consumer.md)), а не клиент.
- **Исполнитель биллинга/снапшота/статуса — consumer, не этот эндпоинт** ([ADR-067 §2](../../adr/ADR-067-agent-run-background-consumer.md)). Правила ([ADR-064](../../adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)/[ADR-066](../../adr/ADR-066-agent-run-state-snapshot.md)) перенесены дословно; пункты ниже описывают **что происходит с прогоном**, независимо от наличия подписчика.
...
1. ~~**`resultText` активного прогона отстаёт**, пока к `/events` никто не подключён. Рецепт клиента: `GET /state` → если `running`/`waiting_approval` → **переподключиться к `/events`** и догнать поток; свежесть снапшота видна по `updatedAt`.~~ ⚠️ **Ревизия 2026-07-29.** До [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md) ограничение было **строже**, чем описано, а рецепт — ненадёжен: он требует, чтобы клиент переподключился, а измерено, что этого не происходит (3 прогона → 0 списаний, прогон висел `running` >15 мин, [TD-037](../../100-known-tech-debt.md)). Перемер 2026-07-30 на активном прогоне ([Q-066-1](../../99-open-questions.md) **Closed**) показал причину: поток событий прогона **одноразовый** — повторная подписка не получает **ни реплея, ни новых событий**, поэтому «догнать поток переподпиской» было невозможно в принципе. **После [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md):** снапшот двигает фоновый consumer, поэтому `resultText` растёт **и без подписчика** — ограничение снимается; остаточное отставание — не более `AGENT_STATE_FLUSH_INTERVAL_SECONDS` (3 с).
...
- **404** — прогон не существует **либо принадлежит другому субъекту** (неотличимы, [06-rbac.md](06-rbac.md)); проверка владения по `agent_runs` — первым шагом обработчика.
- **422** — тело не прошло схему. ⚠️ **Возникает РАНЬШЕ `404` и это не дефект:** ответ одинаков для своего, чужого и несуществующего `runId`, то есть о прогоне не сообщает ничего (как и `401`/`413`/`429`, которые тоже предшествуют проверке владения). Инвариант — «при фиксированной форме запроса ответ не зависит от `runId`» — соблюдён; см. [06-rbac.md](06-rbac.md).
