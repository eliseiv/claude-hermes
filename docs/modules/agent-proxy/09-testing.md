...
⚠️ **Уточнение по двум последним SSE-капчам (выявлено qa при коммите).** Обе — прогоны **заклинившего инстанса**: поток обрывается на `usage.delta`, терминального события нет, `: stream closed` нет. Ранняя постановка описывала их как пару «чистое завершение ↔ контрпример» — **это неверно, на диске оба случая одинаковы**. Дискриминатор `: stream closed` проверяется на реальной паре «капча с `run.completed`» ↔ «капча заклинившего инстанса». Формулировки [Q-067-6](../../99-open-questions.md) этому не противоречат (маркер там отнесён к «чисто завершившемуся потоку»).
...
**E2E (ручной, прод-подобный) — ДВА критерия приёмки, а не один:**
...
2. ⚠️ **Доступность ([TD-039](../../100-known-tech-debt.md)) — обязателен:** сразу после этого **следующий `POST /v1/agent/run` того же пользователя отвечает `202`** в обычные сроки, а не виснет. Без этой проверки приёмка неполна: reaper может поставить корректный статус и списать кумулятив, оставив **заклиненный контейнер**, — то есть БД честна, а пользователь без доступа на 15–20 мин.

✅ **Область критерия 2 сужена замером** ([Q-067-15](../../99-open-questions.md)): проверять **два** пути, а не пять — те, где поток **не отдал ни байта**:

| Путь | Проверять критерий 2? | Основание |
|---|---|---|
| подписка не встала (§1) | **да** | 0 байт ⇒ заклинивает |
| срабатывание guard'а §6.4.2 | **да** | `bytes_read == 0` по определению |
| обрыв установленной подписки | нет | `pd1` (2445 байт) → `202` за 0.238 с |
| зависание / `MAX_DURATION` | нет | consumer получал байты. ⚠️ Случай «зависло при нуле байт» отдельным не является: guard §6.4.2 (180 с) срабатывает **задолго** до `MAX_DURATION` (7200 с), т.е. путь сводится к предыдущей строке |
| **рестарт `api` под прогоном** | нет | тот же случай; партиально дренированный поток безвреден |

⚠️ Для трёх последних это **измеренный факт**, а не допущение: `pd1`/`pd3` + негативный контроль `neg-1` (дочитан до `run.completed`) + контроль изоляции `adr067-ctl` (`202` в каждой точке замера).

**Регресс на дискриминатор (обязателен):** прогон, подписка которого **не получила ни байта** (по образцу вырожденного плеча `pd2`: окно истекло до первого байта) → инстанс заклинивает → orphan-финализация ставит `first_byte_at IS NULL` и **выполняет сброс**; парный негатив — прогон с ≥1 полученным байтом сброса **не** вызывает.

**Предохранители §5.1:** сброс **не** выполняется, если у пользователя есть другой активный прогон **с живым lease или свежим heartbeat** (статуса `running` недостаточно); повторный сброс не чаще `AGENT_RUN_INSTANCE_RESET_COOLDOWN_SECONDS`; ошибка сброса **не** откатывает финализацию статуса и биллинга.

⚠️ **Вне области критерия 2 и вне §5.1 — вход pause-at-zero** ([Q-067-16](../../99-open-questions.md)): статус `paused` терминален, orphan-свип его не отбирает, поэтому §5.1 туда не дотягивается ни при каком триггере. Проверять отдельно **после** замера Q-067-16.
