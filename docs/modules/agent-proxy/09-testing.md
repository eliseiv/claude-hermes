...
- Запустить прогон → убить SSE-потребителя → `GET /state` (статус/текст на момент обрыва) → переподключиться к `/events` → `GET /state` снова. ⚠️ **До [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md) ожидаемый результат этого шага НЕ фиксируется:** он зависит от непроверенной гипотезы H1 ([Q-066-1](../../99-open-questions.md)) — при истинной H1 текст не догонит, при ложной догонит. **Сценарий выполняется в режиме «наблюдать и зафиксировать»**, а не «проверить ожидание»: жёстко заданное ожидание падало бы на корректно работающей системе при ложной H1. Наблюдение приложить к перемеру Q-066-1. **После [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md) ожидание становится определённым и проверяемым:** снапшот двигает consumer, поэтому `resultText` растёт **и во время обрыва** (это уже не зависит от семантики буфера образа), а переподключение к `/events` даёт реплей из нашего ring.

#### Семантика буфера событий Hermes ([Q-066-1](../../99-open-questions.md)) — **ПЕРЕМЕР ОБЯЗАТЕЛЕН**

**Статус: Partially Closed.** Закрыто только сырое наблюдение; интерпретация — гипотеза.

| | Установлено | Статус |
|---|---|---|
| Вторая **последовательная** подписка B (после обрыва A) получила **0 байт**, `run.completed` в B не пришёл | да, наблюдалось | **наблюдение** |
| Это означает дренаж буфера («replay-once-then-drain») | **нет** | **гипотеза H1** |
| Буфер реплеится первому подписчику целиком с начала | **нет** — процедурой не упражнялось | **допущение H4** |
| Образ не поддерживает одновременных подписчиков | **нет** — не измерялось | **гипотеза H2** ([Q-067-5](../../99-open-questions.md)) |

⚠️ **Два дефекта прежней процедуры — оба обязательны к исправлению.**

1. **Конфаундер живости.** Процедура предписывала обрыв A *до* завершения прогона, но **факт продолжения прогона во время B не фиксировался**. Если прогон к моменту B закончился, «0 байт» объясняется тривиально. Сырые артефакты (`runId`, тайминги, счётчики байт, момент завершения) не сохранялись — апостериорно различить нельзя.
2. **Негодный дискриминатор.** `char_length(resultText)` был неприменим: из-за дефекта парсинга `message.delta` (текст лежит в поле **`delta`** и является **строкой** — [05-events.md](05-events.md)) `resultText` был тождественно пуст, и **обе** ветки таблицы исходов давали «текст замер». **Урок методики: дискриминатор не должен зависеть от артефакта, корректность которого сама под вопросом.**

**Корректная процедура (owner: qa; выполнять ПОСЛЕ фикса парсинга `message.delta`).** Дискриминатор — **сырой байтовый поток подписки** плюс **явная проверка живости**.

1. Запустить заведомо длинный прогон (`POST /v1/agent/run`); записать `runId` и `t0`.
2. Подписка A: зафиксировать `t_A_open`, получен ли текст **с начала ответа**, число байт, `t_A_close` (момент обрыва).
3. Оборвать A **заведомо до** завершения прогона.
4. **Проверка живости (обязательна, до выводов).** Прогон обязан продолжаться во время B. **Основной признак — прямой и доступный:** события образа несут **`timestamp` (float, epoch-секунды)** — подтверждено прод-капчей `tests/fixtures/hermes_prod_run_adr065.sse` ([05-events.md](05-events.md)). Достаточно, чтобы B получил событие с `timestamp > t_A_close`. **Резервные признаки** (если B получил 0 байт и сравнивать нечего): рост `usage`/`lastTool` в `GET …/state` после `t_A_close`; либо `run.completed`, пришедшее позже третьей подписке. ⚠️ Именно этот шаг отсутствовал в замере 2026-07-28/29 — и без него «0 байт» неотличимо от «прогон закончился».
5. Подписка B: зафиксировать `t_B_open`, число байт, наличие `run.completed`.
6. Дождаться завершения; зафиксировать `t_end` и итоговый `status` в `/state`.

| Живость подтверждена? | Наблюдение в B | Вывод |
|---|---|---|
| **нет** (`t_end < t_B_open` или живость не доказана) | любое | **Замер невалиден.** Выводов о семантике **не делать**, повторить. Именно этот случай не был исключён в замере 2026-07-28/29 |
| да | **0 байт**, `run.completed` не пришёл | **H1 подтверждена** — `replay-once-then-drain` |
| да | поток с начала, включая виденное в A | H1 опровергнута; семантика `from_start` |
| да | поток идёт, только новые события | H1 опровергнута; семантика `new_only` |

**Отдельная проба H4 (реплей первому подписчику):** подписаться **заведомо позже** первых событий (выждать `≥ 10` с после `202`) и проверить, приходит ли текст **с начала ответа**. Прежняя процедура этого не проверяла никогда.

**Отдельная проба H2 ([Q-067-5](../../99-open-questions.md)):** две **одновременные** подписки на живой прогон; зафиксировать, получают ли обе поток и совпадает ли он.

**Артефакты замера сохранять обязательно** (`runId`, все `t_*`, счётчики байт, **`timestamp` первого и последнего события каждой подписки**, итоговый `status`) — по образцу уже сохранённой капчи `tests/fixtures/hermes_prod_run_adr065.sse` — прикладывать к закрытию [Q-066-1](../../99-open-questions.md). Замер без артефактов не принимается: именно их отсутствие сделало невозможной апостериорную проверку первого прогона.

**На решение [ADR-067](../../adr/ADR-067-agent-run-background-consumer.md) исход не влияет** ([ADR-067 §0](../../adr/ADR-067-agent-run-background-consumer.md)); он влияет на adoption и ретрай подписки ([Q-067-1](../../99-open-questions.md)) и на оценку альтернативы 2.

**Что из старой оснастки остаётся полезным:** `updatedAt` — **не** дискриминатор семантики, а проверка валидности замера (`updated_at = EXCLUDED.updated_at` пишется безусловно, [ADR-066 §6](../../adr/ADR-066-agent-run-state-snapshot.md)). Неподвижный `updatedAt` при живой подписке = замер невалиден (проверить, что `/events` реально открылся — напр. молчаливый `502` wake-gap, [ADR-062](../../adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md)). WARNING `snapshot upsert skipped (tenancy)` виден на `INFO` без изменения уровня логирования.
...
- **Подписка не встала при старте:** `POST /v1/agent/run` всё равно отдаёт `202`, audit `agent_run_consumer_failed`, прогон подхватывает reaper.
