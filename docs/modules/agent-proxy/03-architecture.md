...
- ⚠️ **Ретрай подписки — два разных случая ([ADR-067 §6.4.1](../../adr/ADR-067-agent-run-background-consumer.md)):** обрыв **уже установленной** подписки → ретрай **запрещён навсегда** (поток одноразовый: переподключение ничего не даст, но удержит lease/heartbeat и заблокирует reaper); отказ на **connect**-фазе, когда запрос гарантированно не принят образом → **connect-only retry** по образцу [ADR-062](../../adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md) (явный кортеж `ConnectError`/`ConnectTimeout`/`PoolTimeout`), потому что повторная попытка станет **первой** подпиской, а без подписчика прогон бесплатен навсегда ([TD-037](../../100-known-tech-debt.md)). ⚠️ **Под флагом `AGENT_RUN_CONSUMER_CONNECT_RETRY_ENABLED`, дефолт `false` до замера [Q-067-13](../../99-open-questions.md):** если допущение ложно, ретрай может **успешно установиться транспортно** при инертном потоке, и тогда lease/heartbeat удерживают прогон до `MAX_DURATION` (2 ч) вместо 15 минут — тот же вред, которым обоснован запрет в первом случае.
