...
> **Провайдер-абстракция LLM ([ADR-033](adr/ADR-033-llm-provider-abstraction.md)).** Узел «Anthropic API» на диаграмме — частный случай абстрактного **LLM-провайдера**. Chat Orchestrator вызывает нейтральный `LLMClient` (реализации: `AnthropicClient` / `OpenAIClient`), выбираемый env **`LLM_PROVIDER`, дефолт `anthropic`**. Абстракция позволяет тому же коду работать на любом из провайдеров (**один провайдер на инстанс** — БД хранит wire-формат своего провайдера); `claude-hermes` — anthropic-инстанс (дефолт `LLM_PROVIDER=anthropic`). Мульти-инстанс-развёртывание одного кода на разных провайдерах/доменах — свойство унаследованной от claude-ios кодовой базы, но `claude-hermes` деплоится **единственным инстансом** на выделенном `.156` (`avorelio.shop`, [ADR-057](adr/ADR-057-dedicated-server-self-hosted-traefik-deploy.md)); прочие домены (`broadnova.shop`/`avelyraweb.shop`/`orvianix.shop`/`veltriohub.shop`) — отдельные сервисы, не из этого репозитория. Провайдер-специфичная (де)сериализация — внутри клиента; orchestrator/персист провайдер-агностичны. Детали — [chat-orchestrator/03-architecture.md §Провайдер-абстракция LLM](modules/chat-orchestrator/03-architecture.md#провайдер-абстракция-llm-anthropic--openai-adr-033).
