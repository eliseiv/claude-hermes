...
## Артефакт
Один Docker-образ (multi-stage, base `python:3.12-slim`), запускается через Gunicorn + UvicornWorker. Stateless — состояние в PostgreSQL/Redis. Образ **собирается на сервере** из исходников в `/opt/claude-hermes` (на `.156`; явный `docker compose build api migrate`, затем split-up: обычный `up -d --no-build --no-deps postgres redis traefik` + `up -d --no-build --no-deps --force-recreate api` — см. [§Процедура деплоя](#процедура-деплоя-github-actions--ssh-156)), не пушится из registry ([ADR-057](adr/ADR-057-dedicated-server-self-hosted-traefik-deploy.md), схема «build на сервере» унаследована из [ADR-017](adr/ADR-017-shared-server-traefik-deploy.md)).
...
| `HERMES_IMAGE` | **(Hermes runtime, [ADR-046](adr/ADR-046-per-user-hermes-runtime.md) → источник [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md))** **наш патченый образ Hermes из ghcr, digest-pinned**: `ghcr.io/eliseiv/hermes-agent@sha256:<digest>` (**digest-pin**, НЕ `:latest`, НЕ плавающий тег). **Источник — `ghcr.io/eliseiv` (наш патч-форк), НЕ upstream `nousresearch/hermes-agent`**: upstream не эмитит per-step `usage.delta` + hydrate, обязательные для incremental-биллинга ([ADR-064 §7](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)). **Требование самодостаточности сохранено:** образ собирается **off-server** (dev/CI) и pull'ится на `.156` — сборки из исходников Hermes **на сервере нет** (деплоится только этот репозиторий; docker-py `containers.run(image=...)` авто-pull'ит по digest при отсутствии). ghcr-доступ: **публичный пакет** (анонимный pull, default) либо приватный + `docker login ghcr.io` (см. [§Патченый образ Hermes](#патченый-образ-hermes--сборка-и-публикация-ghcrioeliseiv-adr-065)). Per-instance. **Digest** — для воспроизводимости (образ наш и мутабелен). Предусловие: digest pullable на хосте daemon (`docker pull <ref>`) до первого `/v1/agent/*` или предзагружен. **Дефолт `''` (пусто) → fail-fast: `provision` без заданного образа невозможен** (явная мис-конфигурация). |
...
| `AGENT_STATE_RESULT_TEXT_MAX_CHARS` | **(снапшот agent-прогона, [ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md))** потолок длины **двух** строк снапшота: `agent_run_snapshots.result_text` (текст ответа агента) **и** `pending_approval.preview` (превью действия, ждущего подтверждения, — `_build_pending_approval()`); обе отдаются в `GET /v1/agent/runs/{runId}/state`. Один knob на обе — **обе строки суть user content в снапшоте**, отдельного параметра под preview не вводится. Обрезка **head-preserving** (сохраняется начало) — обрезка хвоста запрещена, на стабильности префикса стоит replay-guard. Тем же лимитом ограничен `output` синтетического SSE `run.paused` ([05-events.md](modules/agent-proxy/05-events.md)). **Дефолт `65536`** (64 KB). Полный текст всегда доступен через `/events`. Не секрет. |
| `AGENT_STATE_FLUSH_INTERVAL_SECONDS` | **(снапшот agent-прогона, [ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md))** троттлинг записи `result_text` в БД на потоке `message.delta`: не чаще одного апсерта в указанный интервал. **Дефолт `3.0`** (сек). Терминальные события (`run.completed`/`run.failed`/`run.paused`) и `approval.request` флашатся **немедленно**, минуя троттлинг. Меньше значение → свежее снапшот и выше нагрузка записи. Не секрет. |
| `AGENT_RUN_SNAPSHOT_TTL_DAYS` | **(снапшот agent-прогона, retention, [ADR-066 §7](adr/ADR-066-agent-run-state-snapshot.md))** возраст, после которого фоновый reaper очищает `result_text=''`/`pending_approval=NULL` у **терминальных** прогонов (`completed`/`failed`/`cancelled`/`paused`). **Строка не удаляется** — `/state` продолжает отдавать `200` со статусом/usage/`updatedAt`; активные (`running`/`resumed`) прогоны не трогаются ни при каком возрасте. **Дефолт `14`** (дней). Это единственный retention для user-content агентных прогонов в БД. Не секрет. |
...
| `HERMES_LAUNCH_RETRY_ATTEMPTS` | **(Hermes runtime, [ADR-062](adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md))** число попыток `POST /v1/runs` в `agent_proxy._launch_run` при **connect-фазовой** transport-ошибке (`ConnectError`/`ConnectTimeout`/`PoolTimeout` — тело не отправлено, дубль-run исключён). Post-send ошибки (`ReadError`/`WriteError`/`RemoteProtocolError`) НЕ ретраятся. **Дефолт `3`** (до 2 доп. попыток). `1` = без retry. Не секрет. |
| `HERMES_LAUNCH_RETRY_BACKOFF_SECONDS` | **(Hermes runtime, [ADR-062](adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md))** backoff между connect-retry попытками `_launch_run`. **Дефолт `2.0`** (сек). Worst-case добавка к латентности неуспешного запуска ≤ `(attempts-1)·backoff`. Не секрет. |
...
- **Образ Hermes `HERMES_IMAGE` — наш патченый образ из ghcr, digest-pinned ([ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md)).** Источник — **`ghcr.io/eliseiv/hermes-agent@sha256:<digest>`** (наш патч-форк, **digest-pin**, НЕ `:latest`), **не** upstream `nousresearch/hermes-agent`: incremental-биллинг ([ADR-064 §7](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)) требует per-step `usage.delta` + hydrate-эндпоинт, которых в upstream нет. **Сборки из исходников Hermes на сервере нет** (инвариант сохранён): образ собирается **off-server** (dev/CI), публикуется в ghcr, а `.156` только pull'ит его по digest (`docker pull <ref>` как предзагрузка либо авто-pull docker-py при `provision`) — дерево Hermes на prod-сервер не попадает. **Зафиксировать digest** (не только тег) для воспроизводимости — образ наш и мутабелен. Процедура сборки/публикации/доставки — [§Патченый образ Hermes](#патченый-образ-hermes--сборка-и-публикация-ghcrioeliseiv-adr-065). Per-instance.
...
- **Retention снапшотов agent-прогонов ([ADR-066 §7](adr/ADR-066-agent-run-state-snapshot.md)):** тот же фоновый reaper-цикл дополнительно очищает `agent_run_snapshots.result_text`/`pending_approval` у **терминальных** прогонов старше `AGENT_RUN_SNAPSHOT_TTL_DAYS` (дефолт 14 дней). Строки не удаляются — `GET /v1/agent/runs/{runId}/state` продолжает отдавать статус/usage; активные прогоны не затрагиваются. **Два обязательных инварианта sweep'а:** (1) **идемпотентность** — guard `AND (result_text <> '' OR pending_approval IS NOT NULL)`, иначе каждый тик (`HERMES_REAPER_INTERVAL_SECONDS`, 300 с) вечно переписывал бы все старые терминальные снапшоты (no-op `UPDATE` → раздувание MVCC-версий, лишний WAL и работа autovacuum); повторный проход обязан давать `rowcount=0`; (2) **`updated_at` НЕ двигается** — чистка контента не является обновлением состояния, иначе ломается клиентский детект устаревания (`updatedAt` в `/state`). Это единственная плановая чистка user-content агентного контура; операционно — следить за ростом таблицы при массовых длинных прогонах (потолок строки — `AGENT_STATE_RESULT_TEXT_MAX_CHARS`).
...
- **Wake readiness-gate + launch-retry ([ADR-062](adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md), фикс прод-`502` «каждый 3–4-й запрос»):** тот же readiness-poll `GET /health` теперь применяется и при **пробуждении** гибернированного контейнера (`docker start`; ранее — немедленный `mark_running` без gate → `POST /v1/runs` уходил в ещё не поднятый `api_server` → transport error → `502`). Wake-таймаут → re-hibernate (`stop`+`mark_stopped`) + `502` (контейнер не удаляется, том цел). Дополнительно `agent_proxy._launch_run` делает **connect-only retry** (`HERMES_LAUNCH_RETRY_ATTEMPTS`=3, backoff `HERMES_LAUNCH_RETRY_BACKOFF_SECONDS`=2.0с) только на connect-фазе — без риска дубль-run (`POST /v1/runs` не идемпотентен). Reuse тех же `HERMES_PROVISION_READY_*` для wake — новых timing-инвариантов нет.
...
- **Health / rollback:** прокси `/v1/agent/*` зависит от доступности инстанса (`health(user_id)`); при недоступности — `502` ([ADR-045 §6](adr/ADR-045-hermes-as-agent-proxy.md)). Rollback образа Hermes — смена `HERMES_IMAGE` digest (на предыдущий опубликованный `ghcr.io/eliseiv/hermes-agent@sha256:<prev-digest>`) + пересоздание инстансов (`deprovision`/`provision`); том сохраняется ([ADR-065 §6](adr/ADR-065-patched-hermes-image-ghcr.md)).
...
## Патченый образ Hermes — сборка и публикация (`ghcr.io/eliseiv`, [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md))
Incremental-биллинг ([ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)) требует, чтобы runtime-образ Hermes эмитил per-step `usage.delta` + отдавал hydrate `GET /api/sessions/{id}/messages` ([ADR-064 §7](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)). Upstream `nousresearch/hermes-agent` этого **не** имеет → используется **наш патченый образ** `ghcr.io/eliseiv/hermes-agent` (digest-pinned). Лицензия Hermes — **MIT** (распространение патченого образа, в т.ч. публичным пакетом, разрешено при сохранении copyright/permission notice).

**Версионирование патча (Вариант B, [ADR-065 §3](adr/ADR-065-patched-hermes-image-ghcr.md)):** diff-патч **`infra/hermes-patches/usage-delta.patch`** хранится в этом репозитории (2 хунка: `agent/conversation_loop.py` — эмиссия `usage.delta`; `gateway/platforms/api_server.py` — проброс в SSE) и применяется на **pinned upstream-базе** при off-server-сборке. Фолбэк — snapshot-форк (git init текущего дерева → отдельный build-source), если upstream-база не пиннится чисто ([Q-046-4](99-open-questions.md)).

**Сборка + публикация (off-server, dev/CI — НЕ на `.156`):**
```
# 1. применить патч на upstream@<pinned-base> (Вариант B) или собрать из snapshot-форка (фолбэк A)
git apply infra/hermes-patches/usage-delta.patch
# 2. сборка под целевую платформу .156 (amd64)
docker build --platform linux/amd64 -t ghcr.io/eliseiv/hermes-agent:usage-delta-v1 .
# 3. логин в ghcr токеном с write:packages (GITHUB_TOKEN/PAT; off-server, НЕ на .156, НЕ в репо)
docker login ghcr.io -u eliseiv
# 4. push + зафиксировать DIGEST из вывода
docker push ghcr.io/eliseiv/hermes-agent:usage-delta-v1
#    -> ghcr.io/eliseiv/hermes-agent@sha256:<digest>   (это и есть значение HERMES_IMAGE)
# 5. (при public-модели) сделать ghcr-пакет public в его настройках
```

**ghcr-доступ на `.156` ([ADR-065 §4](adr/ADR-065-patched-hermes-image-ghcr.md)):**
- **Default — публичный пакет:** `.156` тянет анонимно, **нет секрета на сервере** (паритет с прежней upstream-моделью). Проще всего; MIT позволяет.
- **Альтернатива (приватный пакет):** `docker login ghcr.io -u eliseiv` под **root** на `.156` токеном **`read:packages`** → creds в `/root/.docker/config.json`, docker-py auto-pull при provision подхватывает их из daemon-конфига.
- Финальный выбор visibility — [Q-046-4](99-open-questions.md) (default = public).

**Доставка на `.156` + rollout (последовательность обязательна, [ADR-065 §6](adr/ADR-065-patched-hermes-image-ghcr.md)):**
1. `docker pull ghcr.io/eliseiv/hermes-agent@sha256:<digest>` на `.156` (предзагрузка).
2. `HERMES_IMAGE=ghcr.io/eliseiv/hermes-agent@sha256:<digest>` в `/opt/claude-hermes/.env`.
3. Пересоздать `api` (split-up `--force-recreate api`, [§Процедура деплоя](#процедура-деплоя-github-actions--ssh-156)) — control plane подхватывает новый `HERMES_IMAGE`.
4. **Re-provision** per-user инстансов под новый образ: `deprovision` → `provision` (том `HERMES_HOME` **сохраняется**, [ADR-046](adr/ADR-046-per-user-hermes-runtime.md)).
5. **Включить `agent_incremental_billing_enabled` — ТОЛЬКО ПОСЛЕ** образа на `.156` + **зелёного e2e** агентного пути, где `usage.delta` реально наблюдается в SSE. Флаг OFF (постфактум [ADR-047](adr/ADR-047-usage-based-billing-for-agent.md)) безопасен и остаётся дефолтом до подтверждения.

**Обновление образа (новый патч / bump upstream-базы):** повторить сборку+публикацию → новый digest → шаги доставки 1–4. **Rollback** — вернуть предыдущий опубликованный digest в `HERMES_IMAGE` + re-provision. Синхронизация патча с будущими upstream-обновлениями Hermes — maintenance-долг [TD-036](100-known-tech-debt.md).

**Maintenance-долг:** авто-сборка образа в CI (вместо ручной) — задел, не старт ([TD-036](100-known-tech-debt.md)).
...
- `docker pull ghcr.io/eliseiv/hermes-agent@sha256:<digest>` (наш патченый образ, [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md); при приватном ghcr-пакете — после `docker login ghcr.io`) + задать этот digest-ref в `HERMES_IMAGE` в `.env` (пусто → `provision` fail-fast; **digest-pin, не `:latest`**). E2E incremental-биллинга требует именно патченого образа (в upstream `usage.delta` нет);
...
> - [`infra/legacy/deploy-vps.sh`](../infra/legacy/deploy-vps.sh) — VPS/SSH-специализация под registry+immutable-tag; заменена GitHub Actions SSH workflow (`git pull --ff-only` → explicit `build` → `migrate` → split-up (`up ... postgres redis traefik` + `up ... --force-recreate api`) → readiness-gate, см. [§Процедура деплоя](#процедура-деплоя-github-actions--ssh-156)). DEPRECATED.
...
# (3) start — SPLIT НА ДВА up: force-recreate ТОЛЬКО api (см. §Почему split-up ниже).
#     ПРИЧИНА: `docker compose up -d` решает пересоздавать ли контейнер по CONFIG-hash
#     сервиса, который ссылается на образ по СТАБИЛЬНОМУ тегу (${COMPOSE_PROJECT_NAME}-backend:prod),
#     а НЕ по resolved image id. Пересборка под тем же тегом не меняет config-hash → простой
#     `up -d --no-build` (без явного сервиса) видит "no change" и ОСТАВЛЯЕТ старый контейнер
#     api (старый образ/код), хотя шаг (1) образ пересобрал. /ready (db+redis) при этом зелёный
#     (не отражает версию кода) → зелёный деплой на старом коде.
# (3a) postgres + redis + traefik: ОБЫЧНЫЙ up (стартует, только если не запущены; уже
#      healthy — НЕ трогает). БЕЗ --force-recreate (postgres=данные+downtime, redis=кэш,
#      traefik=держит :443 + ACME/Let's Encrypt state). --no-deps, чтобы traefik (depends_on api)
#      не втянул api в этот шаг.
up_infra_rc=0
$COMPOSE up -d --no-build --no-deps postgres redis traefik || up_infra_rc=$?
# (3b) api ONLY: --force-recreate — контейнер ВСЕГДА принимает свежесобранный образ независимо
#      от неизменного config-hash. --no-deps: зависимости уже удовлетворены (migrate в шаге 2,
#      postgres/redis в 3a) И гарантирует, что --force-recreate НЕ заденет postgres/redis/traefik.
up_rc=0
...
5. **Старт (split-up, без build) — ДВЕ команды:**
   - **(5a) infra — обычный up:** `$COMPOSE up -d --no-build --no-deps postgres redis traefik` — стартует stateful/edge-сервисы, только если они не запущены; уже healthy — **не трогает** (БЕЗ `--force-recreate`). `--no-deps`, чтобы `traefik` (`depends_on: api`) не втянул `api` в этот шаг.
   - **(5b) api — force-recreate:** `$COMPOSE up -d --no-build --no-deps --force-recreate api` — **безусловное** пересоздание **только** `api`, чтобы контейнер принял свежесобранный образ. `--no-deps` гарантирует, что `--force-recreate` **не заденет** `postgres`/`redis`/`traefik`.
   - **rc обеих команд НЕ доверяется** (`|| up_..._rc=$?`; BuildKit-сессия может дать безобидный non-zero). Источник истины — readiness-gate.
   - **Почему split-up + `--force-recreate api` обязателен (root cause прод-инцидента «stale code на зелёном деплое»).** `docker compose up -d` решает, пересоздавать ли контейнер, по **CONFIG-hash сервиса**, который ссылается на образ по **стабильному тегу** `${COMPOSE_PROJECT_NAME}-backend:prod` (= `claude-hermes-backend:prod`), а **не** по resolved image id. Пересборка образа под тем же тегом (шаг 3) **не меняет** config-hash → простой `up -d --no-build` (без явного сервиса) видит «no change» и **оставляет старый контейнер** `api` (старый image id / старый код), несмотря на новый образ и применённую миграцию. `GET /ready` (db+redis) при этом остаётся зелёным — **он не отражает версию кода** (readiness-gate по `/health/ready` не ловит рассинхрон версии), поэтому деплой «зелёный», а на проде исполняется старый код (новые API-поля возвращали `null`). Фикс — `--force-recreate` **только** для `api`.
   - **Инвариант деплоя: `postgres`/`redis`/`traefik` НИКОГДА не под `--force-recreate`.** Их безусловное пересоздание грозит: `postgres` — потеря данных / downtime, `redis` — сброс кэша, `traefik` — переинициализация владельца порта `:443` и **потеря/переоформление Let's Encrypt/ACME state** (`acme.json`). Пересоздаётся только stateless `api`.
6. **Readiness-gate (источник истины):** poll `docker inspect claude-hermes-api-1 --format '{{.State.Health.Status}}'` = `healthy`, 30×2s (≈60s; compose healthcheck = `GET /ready`: db+redis). Не стал healthy → `::error` + `docker logs --tail 40` + `exit 1`. ⚠️ Readiness-gate подтверждает доступность db+redis, но **не версию задеплоенного кода** (см. root cause split-up выше; верификация версии — [TD-034](100-known-tech-debt.md)).
...
- Образ собирается на сервере из исходников (нет immutable registry-tag, [ADR-057](adr/ADR-057-dedicated-server-self-hosted-traefik-deploy.md)). Rollback на `.156` = `git checkout <prev-commit>` в `/opt/claude-hermes` + пересборка/перезапуск (overlay `docker-compose.avorelio.yml` обязателен). Ручной rollback использует ту же последовательность, что и deploy — **build → (при необходимости) migrate → up --force-recreate api**:
  ...
  docker compose -f docker-compose.prod.yml -f docker-compose.avorelio.yml --env-file .env up -d --no-build --no-deps --force-recreate api
  ```
- **`--force-recreate api` при откате ОБЯЗАТЕЛЕН — тот же config-hash footgun, что при деплое** (§Процедура деплоя, шаг 5): пересборка образа отката под стабильным тегом `claude-hermes-backend:prod` не меняет config-hash, поэтому без `--force-recreate` контейнер `api` остался бы на образе НЕ-откаченного коммита. `--no-deps` гарантирует неприкосновенность stateful/edge. `postgres`/`redis`/`traefik` уже подняты и **НЕ** пересоздаются (тот же инвариант: данные / кэш / ACME-cert). Контейнер `traefik` при откате **не** меняется (образ pinned, конфиг в overlay). Миграции expand/contract позволяют откатить код без отката схемы (схема не реверсится — старый код совместим с новой схемой).
...
- [ ] `HERMES_IMAGE` задан **digest-pin нашего патченого образа** `ghcr.io/eliseiv/hermes-agent@sha256:<digest>` (не `:latest`, не upstream `nousresearch/*`, [ADR-065](adr/ADR-065-patched-hermes-image-ghcr.md)); образ **предзагружен** на Docker daemon `.156` (`docker pull <digest>`; при приватном ghcr-пакете — после `docker login ghcr.io`). `HERMES_VOLUME_ROOT` заполнен. `HERMES_LLM_PROVIDER` — валидный провайдер allowlist образа (НЕ `openai`/`auto`), `HERMES_LLM_API_KEY` соответствует провайдеру, `HERMES_MODEL` непуст («голое» имя), `HERMES_LLM_BASE_URL` задан для `custom`/`azure-foundry` ([ADR-055](adr/ADR-055-hermes-instance-llm-config-contract.md), [§Конфигурация (env)](#конфигурация-env)).
