...
erDiagram
    ...
    agent_runs ||--o{ agent_runs : "continued_from"
    agent_runs ||--o| agent_run_snapshots : "state snapshot"
    users ||--o{ agent_run_snapshots : owns
    ...
    agent_runs { text run_id PK }
    agent_run_snapshots { text run_id PK }
```
...
CREATE TABLE hermes_instances (
    ...
    created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),    -- иммутабельное время создания инстанса (НЕ двигается wake/mark_*)
    provisioning_started_at TIMESTAMPTZ                    -- миграция 0017 (ADR-062): начало ТЕКУЩЕЙ provisioning-попытки; stale-якорь _is_stale_provisioning (НЕ created_at)
);
CREATE INDEX ix_hermes_instances_status_active ON hermes_instances (status, last_active_at);
```
> `api_key_enc`/`encrypted_dek`/`nonce` — envelope encryption per-instance `API_SERVER_KEY` (тот же интерфейс `KmsClient`, что BYOK, [ADR-003](adr/ADR-003-byok-envelope-encryption.md)); plaintext ключ — только in-memory на время прокси-вызова ([ADR-045](adr/ADR-045-hermes-as-agent-proxy.md)). `status`: `provisioning` (контейнер создаётся ИЛИ будится/ждёт readiness, [ADR-062](adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md)) → `running` (доступен) → `stopped` (гибернация; том сохранён, будится `ensure_running`). `ix_hermes_instances_status_active` обслуживает фоновый reaper (`stop_idle`: `status='running' AND last_active_at < threshold`). `provisioning_started_at` (миграция `0017`, [ADR-062 §1a](adr/ADR-062-wake-readiness-gate-and-connect-only-launch-retry.md)) — stale-якорь, выставляется на старте каждой provisioning-попытки (cold-start И wake); `created_at` остаётся иммутабельным. Хранение `HERMES_HOME` (тома) — вне БД; политика удаления неактивных томов — [Q-046-2](99-open-questions.md). Детали — [modules/hermes-runtime/04-data-model.md](modules/hermes-runtime/04-data-model.md).

## Таблица agent-run lifecycle (миграция `0018`, [ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md), модуль `agent-proxy`)

### 24. agent_runs (модуль `agent-proxy`)
> **Пошаговый биллинг + pause/resume (миграция `0018`, цепочка `0017`→`0018`, single head, [ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)).** Lifecycle агентного прогона: статус, накопленный расход, точка паузы и цепочка continuation (resume). **Строка пишется всегда** (lifecycle-запись, [ADR-066 §3](adr/ADR-066-agent-run-state-snapshot.md)); под флагом `agent_incremental_billing_enabled` — **только биллинговые поля/операции** (`cumulative_credits_spent`, `last_billed_step`, per-step debits, pause-at-zero, финализация), см. «Ревизию заполнения» ниже. Биллинг-строки — в `ledger_transactions` (source of truth); `agent_runs` — денормализованное зеркало + resume-контракт.
```sql
CREATE TYPE agent_run_status AS ENUM
    ('running', 'paused', 'resumed', 'completed', 'failed', 'cancelled');

CREATE TABLE agent_runs (
    run_id                   TEXT PRIMARY KEY,                    -- Hermes run id (не UUID)
    user_id                  UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    session_id               TEXT NOT NULL,                       -- стабильный ключ resume (одна Hermes-сессия на цепочку)
...
```
> `run_id` — Hermes-строка (PK). `user_id` FK CASCADE (изоляция; RBAC-404 на чужой прогон, [modules/agent-proxy/06-rbac.md](modules/agent-proxy/06-rbac.md)). `session_id` стабилен на всю continuation-цепочку (`effective_session_id = body.sessionId or uuid4()`, [ADR-064 §5](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)). `status` переходы: `running → {paused,completed,failed,cancelled}`, `paused → resumed` (resume создаёт child с `continued_from_run_id = parent`). `continued_from_run_id` self-FK `ON DELETE SET NULL`; корень цепочки — `NULL`. Миграция `0018` (revision id `0018_agent_runs` ≤32 символов; `down_revision="0017_hermes_provisioning"` — полный id; upgrade+downgrade к реальной БД). Детали — [modules/agent-proxy/02-api-contracts.md](modules/agent-proxy/02-api-contracts.md), [modules/agent-proxy/05-events.md](modules/agent-proxy/05-events.md).

> **Ревизия заполнения ([ADR-066 §3](adr/ADR-066-agent-run-state-snapshot.md), 2026-07-28) — строка пишется ВСЕГДА.** `agent_runs` перестаёт быть «строкой под флагом биллинга» и становится **безусловной lifecycle-записью**: `create_running` (корневой прогон + resume-child) и запись терминального статуса выполняются независимо от `agent_incremental_billing_enabled`. Под флагом остаются **только биллинговые поля/операции**: `cumulative_credits_spent`, `last_billed_step`, per-step debits, pause-at-zero, финализация остатка. Схема таблицы и enum `agent_run_status` **не меняются** (миграции нет). Дополнительно начинают выставляться статусы, которые до сих пор существовали в enum, но никогда не записывались: **`failed`** (на событии `run.failed`) и **`cancelled`** (после 2xx клиентского `POST /v1/agent/runs/{runId}/stop`). Мотив: без этого на дефолтной конфигурации (флаг OFF) агентных прогонов в БД нет вовсе, а упавший/остановленный прогон вечно числится `running`.
>
> **Все переходы статуса — условные:** `UPDATE … WHERE run_id=:id AND status IN ('running','resumed')` — включая `completed` и `failed`, не только `cancelled`. Безусловная запись терминального статуса создаёт гонку: после `POST /stop` Hermes ещё дофлашивает события в открытый relay, и долетевшее `run.completed`/`run.failed` затёрло бы `cancelled`. Условие делает победителем **первый** терминальный статус и защищает `paused` от затирания. **`mark_stopped(run_id, user_id)` (owner-scoped, `AND user_id=:uid`) вызывается только на клиентском пути `/stop`** — внутренний Hermes-interrupt при pause-at-zero ([ADR-064 §3](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)) идёт тем же `stop()`, и общая пометка статуса транзиентно давала бы `cancelled` вместо `paused` (сломав `/state` и `/resume`).

## Таблица снапшота состояния agent-прогона (миграция `0019`, [ADR-066](adr/ADR-066-agent-run-state-snapshot.md), модуль `agent-proxy`)

### 25. agent_run_snapshots (модуль `agent-proxy`)
> **Снапшот состояния прогона для `GET /v1/agent/runs/{runId}/state` (миграция `0019`, цепочка `0018`→`0019`, single head, [ADR-066](adr/ADR-066-agent-run-state-snapshot.md)).** 1:1 к `agent_runs`, отдельной таблицей — у строк разный профиль записи (снапшот апсертится десятки раз за прогон против единиц у lifecycle-строки), разный retention и разная чувствительность (снапшот содержит user-facing текст модели). Заполняется **побочным эффектом SSE-relay** `/events` ([modules/agent-proxy/05-events.md](modules/agent-proxy/05-events.md)), независимо от флага `agent_incremental_billing_enabled`.
```sql
CREATE TABLE agent_run_snapshots (
    run_id           TEXT PRIMARY KEY REFERENCES agent_runs(run_id) ON DELETE CASCADE,
    user_id          UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,   -- скоуп RBAC + CASCADE при удалении пользователя
    result_text      TEXT NOT NULL DEFAULT '',                               -- склейка message.delta, head-preserving truncation
    last_tool        TEXT,                                                   -- последний tool.started/tool.completed
    pending_approval JSONB,                                                  -- {"tool": "...", "preview": "..."}; NULL = не ждёт подтверждения
                                                                             -- preview обрезан тем же AGENT_STATE_RESULT_TEXT_MAX_CHARS (head-preserving), что и result_text
    input_tokens     BIGINT NOT NULL DEFAULT 0 CHECK (input_tokens  >= 0),
    output_tokens    BIGINT NOT NULL DEFAULT 0 CHECK (output_tokens >= 0),
    updated_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- ADR-067 §4 (миграция 0020): heartbeat фонового consumer'а. НЕ отдаётся клиенту.
    -- Отдельная колонка обязательна: updated_at этой таблицы И agent_runs.updated_at оба
    -- служат клиентским детектором устаревания (/state.updatedAt, ADR-066 §5), а heartbeat
    -- идёт и тогда, когда состояние не менялось → двигать их значило бы врать клиенту.
    consumer_heartbeat_at TIMESTAMPTZ
    -- ADR-067: колонка first_byte_at ОТМЕНЕНА вместе с механизмом расклинивания §5.1 —
    -- его предпосылка (Факт 3: недренированный поток заклинивает инстанс) опровергнута:
    -- заклинивался наш row-lock, а не инстанс (TD-039 переатрибутирован, TD-040 закрыт).
);
-- ADR-067 §5: рабочее множество orphan-свипа — АКТИВНЫЕ прогоны; предикат самоочищающийся
-- (терминальный прогон выпадает из индекса навсегда), поэтому индекс не растёт с историей.
-- Ведущий столбец created_at, НЕ updated_at: возраст кандидата считается по
-- COALESCE(snapshot.consumer_heartbeat_at, agent_runs.created_at), а updated_at в свипе
-- не участвует вовсе. Роль индекса — компактное partial-множество активных прогонов
-- + устойчивый oldest-first порядок под потолок AGENT_RUN_ORPHAN_MAX_PER_TICK.
CREATE INDEX ix_agent_runs_active ON agent_runs (created_at)
    WHERE status IN ('running', 'resumed');
CREATE INDEX ix_agent_run_snapshots_user  ON agent_run_snapshots (user_id);
-- ADR-067 §5: ОТДЕЛЬНОГО индекса по consumer_heartbeat_at НЕТ (см. ремарку ниже) —
-- orphan-свип стартует от agent_runs (активные прогоны) и джойнит снапшот по PK.
-- retention-sweep: ЧАСТИЧНЫЙ индекс, предикат совпадает с guard'ом sweep-запроса →
-- очищенные строки выпадают из индекса, в установившемся режиме тик reaper'а стоит O(0)
CREATE INDEX ix_agent_run_snapshots_sweep ON agent_run_snapshots (updated_at)
    WHERE result_text <> '' OR pending_approval IS NOT NULL;
```
> **Статус здесь НЕ дублируется** — источник истины остаётся `agent_runs.status` (иначе два места правды и неизбежный рассинхрон); клиентский `waiting_approval` — **производный** (`status ∈ {running,resumed}` AND `pending_approval IS NOT NULL`), значением enum он не является, `agent_run_status` **не расширяется**. `run_id` — PK и одновременно FK на `agent_runs` CASCADE (снапшот не переживает lifecycle-строку). `user_id` дублируется для прямого RBAC-скоупа и CASCADE от `users`. **Апсерт `ON CONFLICT (run_id) DO UPDATE` с replay-guard — per-column, НЕ row-level** ([ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md)): `result_text = CASE WHEN char_length(EXCLUDED.result_text) >= char_length(t.result_text) AND left(EXCLUDED.result_text, char_length(t.result_text)) = t.result_text THEN EXCLUDED.result_text ELSE t.result_text END` (**префиксный** guard — длины мало: она ловит укорачивание, но пропустила бы более длинную **подмену**; при отсутствии replay текст замерзает вместо порчи, [Q-066-1](99-open-questions.md)), `input_tokens`/`output_tokens` = `GREATEST(t.…, EXCLUDED.…)`, `pending_approval = CASE WHEN :assert_approval THEN EXCLUDED.pending_approval ELSE t.pending_approval END`, а `last_tool`/`updated_at` — **безусловно** `EXCLUDED.*`. ⚠️ **Ремарка 2026-07-29:** посылка «Hermes при переподключении реплеит буфер с начала» — **непроверенное допущение** (H4, [Q-066-1](99-open-questions.md)), прод-процедурой не упражнялась; guard корректен при любой семантике, поэтому SQL не меняется, но ссылаться на неё как на факт нельзя. ⚠️ **Row-level `WHERE` у `DO UPDATE` запрещён** — он гейтит строку целиком, из-за чего в replay-окне не записались бы ни `pending_approval` (клиентский `waiting_approval` не появился бы никогда), ни `last_tool`, ни `updated_at`. Гейт `pending_approval` — **по семантике источника** (`assert_pending_approval=true` только у `approval.request`/`tool.*`/терминальных/`POST …/approval`; троттлинговый флаш `message.delta` идёт с `false`), иначе периодический флаш текста воскресил бы уже снятый approval. `result_text` **и** `pending_approval.preview` обрезаются **с головы** одним и тем же `AGENT_STATE_RESULT_TEXT_MAX_CHARS` (64 KB; обе строки — user content, отдельного knob'а под preview нет), запись троттлится `AGENT_STATE_FLUSH_INTERVAL_SECONDS` (3.0 с). Снятие approval после 2xx `POST …/approval` — отдельный owner-scoped `clear_pending_approval(run_id, user_id)` (чистый `UPDATE`, без `INSERT`; единственный writer снапшота вне relay-пути). **Tenancy-гвард:** апсерт несёт row-level `WHERE t.user_id = EXCLUDED.user_id`, чтение — `snapshots_repo.get(run_id, user_id)` (`AND user_id=:uid`); это defense-in-depth под допущение о **глобальной уникальности** Hermes `run_id` ([Q-066-2](99-open-questions.md)) — `run_id` генерируется инстансом каждого пользователя независимо, а PK здесь глобальный, поэтому при гипотетической коллизии чужая строка не читается и не перезаписывается (`rowcount=0` / `None`). Тенантный предикат — гейт по **identity**, а не по свежести данных, и запрету row-level staleness-`WHERE` не противоречит ([ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md)). **Retention `AGENT_RUN_SNAPSHOT_TTL_DAYS` (14 дней):** reaper очищает `result_text=''`/`pending_approval=NULL` у **терминальных** прогонов старше TTL — **строка не удаляется**, `/state` продолжает отдавать статус/usage/`updatedAt`; активные (`running`/`resumed`) прогоны не трогаются. Sweep **идемпотентен** (guard `AND (result_text <> '' OR pending_approval IS NOT NULL)` — повторный проход даёт `rowcount=0`), обслуживается **частичным** `ix_agent_run_snapshots_sweep` (предикат индекса = guard запроса → очищенные строки выпадают из индекса, в установившемся режиме тик стоит O(0)) и **не двигает `updated_at`** (иначе поле перестало бы означать «время последней записи состояния» и сломало бы клиентский детект устаревания). Настройки — [07-deployment.md](07-deployment.md). Миграция `0019` (revision id `0019_agent_run_snapshots` ≤32 символов; `down_revision="0018_agent_runs"`; expand-only, upgrade+downgrade к реальной БД). **Миграция `0020`** ([ADR-067 §4](adr/ADR-067-agent-run-background-consumer.md), `down_revision="0019_agent_run_snapshots"`, expand-only): **одна** колонка `agent_run_snapshots.consumer_heartbeat_at TIMESTAMPTZ NULL` (живость consumer'а) + **частичный индекс `ix_agent_runs_active` на `agent_runs`**. ⚠️ Колонка `first_byte_at` **отменена** вместе с механизмом [ADR-067 §5.1](adr/ADR-067-agent-run-background-consumer.md) — его предпосылка опровергнута ([TD-039](100-known-tech-debt.md) переатрибутирован). Колонка **не отдаётся клиенту** и в контракт `/state` не входит.

> ⚠️ **Почему индекса по `consumer_heartbeat_at` НЕТ (ревизия решения «итого два индекса», [ADR-066 §7](adr/ADR-066-agent-run-state-snapshot.md)).** Первый вариант заводил третий индекс `ix_agent_run_snapshots_hb … WHERE consumer_heartbeat_at IS NOT NULL` — **отвергнут**: (1) его предикат **не самоочищается** (истинен для каждой тронутой consumer'ом строки), поэтому индекс растёт со всей историей прогонов — ровно то свойство, ради отсутствия которого [ADR-066 §7](adr/ADR-066-agent-run-state-snapshot.md) отверг полный индекс по `updated_at`; (2) он даёт write-amplification на **горячей** колонке (запись раз в 30 с на активный прогон); (3) планировщик всё равно пошёл бы от `agent_runs` — рабочее множество свипа задаётся статусом, а не heartbeat'ом. Вместо него — `ix_agent_runs_active` с предикатом **по статусу**: он самоочищается (терминальный прогон выпадает навсегда), покрывает именно то множество, которое свип перебирает, и живёт в таблице, куда heartbeat **не** пишется. **Ведущий столбец — `created_at`, а не `updated_at`:** возраст кандидата считается по `COALESCE(snapshot.consumer_heartbeat_at, agent_runs.created_at)`, `updated_at` в свипе не участвует вовсе; индекс работает как **partial-множество** активных прогонов и заодно даёт oldest-first порядок под потолок `AGENT_RUN_ORPHAN_MAX_PER_TICK`. **Состав индексов `agent_run_snapshots` остаётся прежним — два** (`ix_agent_run_snapshots_user` + частичный `ix_agent_run_snapshots_sweep`), инвариант [ADR-066 §7](adr/ADR-066-agent-run-state-snapshot.md) не нарушен.

> **Инвариант heartbeat-записи ([ADR-067 §4](adr/ADR-067-agent-run-background-consumer.md)):** heartbeat пишется **ВСЕГДА** отдельным `UPDATE agent_run_snapshots SET consumer_heartbeat_at = now() WHERE run_id = :id` — ровно одна колонка, **без исключений для случая, когда события идут**. Использовать для этого обычный апсерт снапшота **запрещено**: он пишет `updated_at = EXCLUDED.updated_at` безусловно, и heartbeat-only-флаш двигал бы клиентский детектор устаревания без изменения состояния (симметрично инварианту 2 retention-sweep'а — «`updated_at` sweep не трогает»). Детали — [modules/agent-proxy/02-api-contracts.md](modules/agent-proxy/02-api-contracts.md), [modules/agent-proxy/05-events.md](modules/agent-proxy/05-events.md).
...
- **Incremental agent-billing + agent_runs ([ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)):** на incremental-пути (флаг `agent_incremental_billing_enabled`) списание идёт **per-step** на `usage.delta` — отдельный подкласс agent-debit: `idempotency_key = f"{runId}:{stepIndex}"`, `meta.source='agent_run'` + `meta.incremental=true`, `amount = min(want, balance)` где `want = usage_to_credits(cumulative) − charged` (телескопирует **точно** в `usage_to_credits(final)`, [ADR-064 §1](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)); финализация остатка на `run.completed` — `idempotency_key = runId` (голый, отдельное пространство ключей, не конфликтует с `runId:step`). **No-debt инвариант:** incremental-consume клэмпит `balance − LEAST(:amount, balance)` → `wallets.debt` **не** растёт (ветка [ADR-051](adr/ADR-051-agent-debt-reconciliation.md) обходится по `meta.incremental`); долг возможен только на finalization-гонке / flag-off (fallback). При исчерпании — `agent_runs.status='paused'` **без** debt. **Инвариант зеркала:** `agent_runs.cumulative_credits_spent == Σ ledger.amount` по debit-строкам `idempotency_key = runId OR LIKE 'runId:%'` этого прогона; ledger — источник истины, зеркало пересчитывается через `charged_for_run` (reconnect-safe seed). Resume создаёт НОВЫЙ прогон со **свежим** keyspace `newRunId:step` — двойного счёта нет.
- **Снапшот agent-прогона ([ADR-066](adr/ADR-066-agent-run-state-snapshot.md)):** `agent_run_snapshots` — **производные** данные (реконструируются из потока событий), источником истины **не являются**: статус живёт в `agent_runs.status`, деньги — в `ledger_transactions`, usage-каунты снапшота носят информационный характер и на биллинг не влияют. **Монотонность записи:** `result_text` растёт только вперёд (**префиксный** replay-guard, [ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md)), токены — через `GREATEST`; поэтому повторная запись не «откатывает» снапшот. ⚠️ Мотивировка «Hermes реплеит буфер с начала при переподключении» — **непроверенное допущение H4** ([Q-066-1](99-open-questions.md), та же пометка в §25 выше); guard корректен при любой семантике, но ссылаться на неё как на факт нельзя. `result_text` ограничен `AGENT_STATE_RESULT_TEXT_MAX_CHARS` (head-preserving) — **полнота текста в БД не гарантируется**. ⚠️ **Ревизия 2026-07-29 ([ADR-067](adr/ADR-067-agent-run-background-consumer.md)):** формулировка «полный текст всегда доступен через `/events`» была верна для прямого relay и **под broker-моделью неверна** — ring тоже ограничен (`AGENT_RUN_EVENT_BUFFER_MAX`/`..._MAX_BYTES`), ради чего и введён обязательный маркер `run.truncated`. **Полноту текста длинного прогона не гарантирует ни один источник:** `/state.resultText` — 64 KB head-preserving, `/events` — содержимое ring'а текущего поколения. Неполнота при этом **наблюдаема** (маркер), а не молчалива. **Retention:** у терминальных прогонов старше `AGENT_RUN_SNAPSHOT_TTL_DAYS` `result_text`/`pending_approval` очищаются, строка остаётся (`/state` продолжает отдавать статус/usage). **Свежесть:** ⚠️ **устарело с [ADR-067](adr/ADR-067-agent-run-background-consumer.md)** — снапшот двигает **фоновый consumer**, а не клиентский relay, поэтому он растёт и у прогона без подписчика (ради этого ADR-067 и вводился: до него прогон без подписчика вдобавок не тарифицировался, [TD-037](100-known-tech-debt.md)). Остаточное отставание — не более `AGENT_STATE_FLUSH_INTERVAL_SECONDS` (3 с). **Профиль записи:** `consumer_heartbeat_at` обновляется раз в `AGENT_RUN_CONSUMER_HEARTBEAT_SECONDS` (30 с) **отдельным одноколоночным `UPDATE`** (`SET consumer_heartbeat_at = now() WHERE run_id = :id`) — см. **инвариант heartbeat-записи** выше в этом же §25 и [ADR-067 §4](adr/ADR-067-agent-run-background-consumer.md). ⚠️ **Дополнительный round-trip ЕСТЬ — это осознанная цена**, а не побочный эффект: обычный апсерт снапшота пишет `updated_at = EXCLUDED.updated_at` **безусловно** ([ADR-066 §6](adr/ADR-066-agent-run-state-snapshot.md)), поэтому heartbeat через апсерт двигал бы клиентский детектор устаревания без изменения состояния — та самая утечка в `/state.updatedAt`, ради устранения которой и заводилась отдельная колонка. Использовать апсерт для heartbeat **запрещено**. **Запись идёт в эту таблицу, а не в `agent_runs`** — иначе «денежная» строка стала бы горячей и обоснование разделения таблиц ([ADR-066 §2](adr/ADR-066-agent-run-state-snapshot.md): раздельные профили записи и retention) обнулилось бы.
