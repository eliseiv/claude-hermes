...
> **Пошаговый биллинг + pause/resume (миграция `0018`, цепочка `0017`→`0018`, single head, [ADR-064](adr/ADR-064-incremental-agent-run-billing-and-pause-resume.md)).** Lifecycle агентного прогона: статус, накопленный расход, точка паузы и цепочка continuation (resume). Заполняется на incremental-пути (флаг `agent_incremental_billing_enabled`). Биллинг-строки — в `ledger_transactions` (source of truth); `agent_runs` — денормализованное зеркало + resume-контракт.
```sql
