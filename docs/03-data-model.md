...
- **Снапшот agent-прогона ([ADR-066](adr/ADR-066-agent-run-state-snapshot.md)):** `agent_run_snapshots` — **производные** данные (реконструируются из потока событий), источником истины **не являются**: статус живёт в `agent_runs.status`, деньги — в `ledger_transactions`, usage-каунты снапшота носят информационный характер и на биллинг не влияют. **Монотонность записи:** `result_text` растёт только вперёд (replay-guard `char_length(EXCLUDED.result_text) >= char_length(existing)` — Hermes реплеит буфер с начала при переподключении), токены — через `GREATEST`; поэтому повторная подписка на `/events` не «откатывает» снапшот. `result_text` ограничен `AGENT_STATE_RESULT_TEXT_MAX_CHARS` (head-preserving) — **полнота текста в БД не гарантируется**, полный ответ доступен только через `/events`. **Retention:** у терминальных прогонов старше `AGENT_RUN_SNAPSHOT_TTL_DAYS` `result_text`/`pending_approval` очищаются, строка остаётся (`/state` продолжает отдавать статус/usage). **Свежесть:** снапшот двигается только при живом relay — для активного прогона без подписчика он отстаёт (детект по `updated_at`).
