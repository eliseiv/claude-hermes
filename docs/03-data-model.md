...
CREATE TABLE agent_run_snapshots (
    ...
    -- ADR-067 §5.1.2 (та же миграция 0020): момент ПЕРВОГО байта, полученного подпиской
    -- consumer'а. NULL = поток не отдал ничего → признак заклинивания инстанса (TD-039),
    -- по которому orphan-свип решает, нужен ли принудительный сброс контейнера.
    -- Проксировать «пустым снапшотом» нельзя: байты могли прийти событием, которое в
    -- снапшот не накапливается (reasoning.available), и прогон выглядел бы нулевым.
...
> **Инвариант heartbeat-записи ([ADR-067 §4](adr/ADR-067-agent-run-background-consumer.md)):** при длительном молчании прогона heartbeat пишется **отдельным `UPDATE agent_run_snapshots SET consumer_heartbeat_at = now() WHERE run_id = :id`** — ровно одна колонка. Использовать для этого обычный апсерт снапшота **запрещено**: он пишет `updated_at = EXCLUDED.updated_at` безусловно, и heartbeat-only-флаш двигал бы клиентский детектор устаревания без изменения состояния (симметрично инварианту 2 retention-sweep'а — «`updated_at` sweep не трогает»). Детали — [modules/agent-proxy/02-api-contracts.md](modules/agent-proxy/02-api-contracts.md), [modules/agent-proxy/05-events.md](modules/agent-proxy/05-events.md).
