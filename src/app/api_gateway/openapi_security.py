...
user_id_scheme = APIKeyHeader(
    ...
    auto_error=False,
    description=(
        "Стабильный идентификатор пользователя (каноническая UUID-строка используется как "
        "есть; иная строка детерминированно маппится в UUID). Идентичность доверяется (ключ "
        "доверенный). Обязателен вместе с `X-API-Key` — вставьте идентификатор субъекта в "
        "заголовок `X-User-Id` через Authorize."
    ),
)
