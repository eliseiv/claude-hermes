...
# ADR-059 §7 (Reversed): full OpenAPI surface restored — every active router is documented, ordered
# by the pre-ADR-059 tag order with `Auth` removed (retired /v1/auth/*, ADR-044 §4a) and
# `Billing (Adapty)` inserted between BYOK and Admin. `Subscription` stays retired (TD-021).
_OPENAPI_TAGS = [
    {
        "name": "Agent",
        "description": (
...
def create_app() -> FastAPI:
    ...
    # ADR-044 §4a: the client contour uses X-API-Key + X-User-Id; the retired JWT/Apple issuer
    # (/v1/auth/*) is NOT wired (404, out of schema). The auth_refresh_tokens cleanup reaper
    # (TD-013) still runs. ADR-059 §7 (Reversed): hide-only is cancelled — the OpenAPI surface is
    # full again (every active router is documented, minus retired /v1/auth/*). All routers below,
    # including the nine claude-ios legacy ones, register as visible via a single common loop.
    for module in (
        agent,
        policy,
        wallet,
